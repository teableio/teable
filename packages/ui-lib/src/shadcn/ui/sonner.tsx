...
const Toaster = ({ expand, visibleToasts, ...props }: ToasterProps) => {
  const { theme = 'system' } = useTheme();
  const { toasts } = useSonner();
  const [isHovered, setIsHovered] = useState(false);

  useEffect(() => {
    // Removing the final toast can unmount the hover target without a mouseleave event.
    if (toasts.length === 0) {
      setIsHovered(false);
    }
  }, [toasts.length]);
  ...
  return (
    <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <Sonner
        ...
        richColors
        // Default expansion only changes layout; Sonner keeps each toast's timer running.
        expand={expand ?? toasts.length <= STACK_THRESHOLD}
        visibleToasts={visibleToasts ?? (isHovered ? Infinity : STACK_THRESHOLD)}
        ...
        toastOptions={{
          classNames: {
            ...
            icon: 'size-5 items-center justify-center !ml-0 [&_svg]:!ml-0 [&_svg]:!mr-0 me-0 [.toast:has([data-description])_&]:col-start-1 [.toast:has([data-description])_&]:row-start-1',
            closeButton:
              'static order-last ms-1 h-5 w-5 shrink-0 self-center border-none !bg-transparent !text-muted-foreground hover:!text-foreground [transform:none] [&_svg]:size-4 [.toast:has([data-description])_&]:col-start-3 [.toast:has([data-description])_&]:row-start-1 [.toast:has([data-description])_&]:ms-0 [.toast:has([data-description])_&]:justify-self-end [.toast:has([data-description])_&]:self-start [.toast:has([data-description]):has([data-action]):has([data-cancel])_&]:col-start-4',
            content:
        ...
        // Clears the fixed top-banner stack; the CSS var is 0 when no banner shows.
        offset={props.offset ?? { top: 'calc(var(--teable-top-banner-height, 0px) + 32px)' }}
        {...props}
      />
    </div>
  );
