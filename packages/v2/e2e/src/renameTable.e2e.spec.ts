...
import { createTableOkResponseSchema, renameTableOkResponseSchema } from '@teable/v2-contract-http';
...
describe('v2 http renameTable (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    tableId = await createTable('Rename Me');
    secondTableId = await createTable('Rename Me Too');
  });
  ...
  it('returns ok via orpc client', async () => {
    ...
    const body = await client.tables.rename({ baseId, tableId: secondTableId, name: 'Renamed' });
