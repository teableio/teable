...
let testContainer: IV2NodeTestContainer | undefined;

const getTestContainer = (): IV2NodeTestContainer => {
  if (!testContainer) {
    throw new Error('Computed optimization test container is not initialized');
  }
...
describe('ComputedOptimization (e2e)', () => {
  ...
  describe('insert optimization', () => {
    it('creates record on One-side table (bidirectional link)', async () => {
      const { container, baseId } = getTestContainer();
  ...
  it('batches lookup updates for multiple linked records', async () => {
    const { container, baseId, processOutbox, eventBus } = getTestContainer();
    const commandBus = container.resolve<ICommandBus>(v2CoreTokens.commandBus);
