...
describe('v2 field explain endpoints (e2e)', () => {
  ...
  it('snapshots anonymized formula SQL diagnostics without running SQL explain', async () => {
    ...
    try {
      ...
      const updatedLine = (await ctx.listRecords(lineTable.id)).find(
        (record) => record.id === lineRecord.id
      );
      const expectedDeltaReason =
        'Price (6 units): billed 3.6/u vs reference 3/u. Quantity (6 units): 10 planned, 8 actual.';
      const expectedActionReason =
        'billed amount (36 units) is greater than actual reference value (24.00 units)';
      const expectedDeltaExplanation = `\n* item alpha updated (code SKU-1) : delta 12 units. ${expectedDeltaReason}`;
      const expectedActionLineText = `\n- item alpha updated (code SKU-1) : billed 36 units, actual value 24.00 units (reference) -> delta 12 units -- ${expectedActionReason}`;
      expect(updatedLine?.fields[priceDeltaFieldId]).toBe(6);
      expect(updatedLine?.fields[quantityDeltaFieldId]).toBe(6);
      expect(updatedLine?.fields[amountDeltaFieldId]).toBe(12);
      expect(updatedLine?.fields[lineStateFieldId]).toBe('delta');
      expect(updatedLine?.fields[isDeltaFieldId]).toBe(true);
      expect(updatedLine?.fields[actionEligibleFieldId]).toBe(true);
      expect(updatedLine?.fields[actionAmountFieldId]).toBe(12);
      expect(updatedLine?.fields[severityFieldId]).toBe('minor');
      expect(updatedLine?.fields[referenceUnitPriceFieldId]).toBe(3);
      expect(updatedLine?.fields[billedUnitPriceFieldId]).toBe(3.6);
      expect(updatedLine?.fields[deltaReasonFieldId]).toBe(expectedDeltaReason);
      expect(updatedLine?.fields[actionReasonFieldId]).toBe(expectedActionReason);
      expect(updatedLine?.fields[deltaExplanationFieldId]).toBe(expectedDeltaExplanation);
      expect(updatedLine?.fields[actionLineTextFieldId]).toBe(expectedActionLineText);

      const updatedParent = (await ctx.listRecords(parentTable.id)).find(
        (record) => record.id === parentRecord.id
      );
      expect(updatedParent?.fields[parentDeltaRollupFieldId]).toContain(expectedDeltaExplanation);
      expect(updatedParent?.fields[parentActionRollupFieldId]).toContain(expectedActionLineText);
      expect(updatedParent?.fields[parentSummaryFormulaFieldId]).toContain(
        expectedDeltaExplanation
      );
    } finally {
