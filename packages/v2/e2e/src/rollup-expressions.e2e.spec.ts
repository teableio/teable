  ...
  describe('rollup targeting conditional computed fields', () => {
    ...
    it('rolls up conditionalRollup and conditionalLookup values across linked tables', async () => {
      ...
      try {
        ...
        expect(northRecord?.fields[rootItemConcatRollupFieldId]).toEqual('Alpha');
        expect(globalRecord?.fields[rootItemConcatRollupFieldId]).toEqual('Alpha, Gamma');
        expect(unlinkedRecord?.fields[rootItemConcatRollupFieldId] ?? null).toBeNull();
      } finally {
