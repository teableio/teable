...
describe('v2 http createRecord link fields (e2e)', () => {
  ...
  describe('manyMany relationship', () => {
    ...
    it('creates record with multiple links and verifies via listRecords', async () => {
      ...
      const mainRecord = await createRecord(mainTableId, {
  ...
  describe('manyOne relationship', () => {
    ...
    beforeAll(async () => {
      ...
      const fr = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Parent Record' });
    ...
    it('creates record with single link and verifies in database', async () => {
      ...
      const result = await sql<{ fk_value: string | null }>`
        ...
        FROM ${sql.table(`${baseId}.${mainTableId}`)}
    ...
    it('creates multiple records linking to same parent and verifies in database', async () => {
      ...
      const childRecord = await createRecord(mainTableId, {
  ...
  describe('oneOne relationship', () => {
    ...
    beforeAll(async () => {
      ...
      // Create record in foreign table
      const fr = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Unique Partner' });
      foreignRecordId = fr.id;
    ...
    it('FK value is consistent across multiple database reads', async () => {
      ...
      const newForeignRecord = await createRecord(foreignTableId, {
  ...
  describe('oneMany relationship (one-way junction)', () => {
    ...
    beforeAll(async () => {
      ...
      // Create records in foreign table
      const fr1 = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Child 1' });
      const fr2 = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Child 2' });
      foreignRecordId1 = fr1.id;
      foreignRecordId2 = fr2.id;
  ...
  describe('self-referential links', () => {
    describe('self manyMany', () => {
      ...
      beforeAll(async () => {
        ...
        const r1 = await createRecord(tableId, { [titleFieldId]: 'Person A' });
        const r2 = await createRecord(tableId, { [titleFieldId]: 'Person B' });
        recordId1 = r1.id;
        recordId2 = r2.id;

        // Add self-referential link field
        const response = await fetch(`${baseUrl}/fields/create`, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            tableId,
    ...
    describe('self manyOne (hierarchical)', () => {
      ...
      beforeAll(async () => {
        ...
        const parent = await createRecord(tableId, { [titleFieldId]: 'Parent Node' });
      ...
      it('creates child record with parent link and verifies', async () => {
        ...
        const childRecord = await createRecord(tableId, {
  ...
  describe('empty and null link values', () => {
    ...
    it('creates record with empty array link', async () => {
      const record = await createRecord(mainTableId, {
      ...
      expect(found).toBeDefined();
      // Empty link should result in null or empty array
      const linkValue = found?.fields[linkFieldId];
      expect(linkValue === null || (Array.isArray(linkValue) && linkValue.length === 0)).toBe(true);
    });
  ...
  describe('multiple link fields on same table', () => {
    ...
    beforeAll(async () => {
      ...
      const foreignTable1 = await createTable({
        baseId,
      ...
      const fr1 = await createRecord(foreignTable1Id, { [foreign1TitleFieldId]: 'Target 1' });
      foreign1RecordId = fr1.id;

      // Create second foreign table
      const foreignTable2 = await createTable({
        baseId,
      ...
      const fr2 = await createRecord(foreignTable2Id, { [foreign2TitleFieldId]: 'Target 2' });
  ...
  describe('database verification', () => {
    describe('manyOne FK column in database', () => {
      ...
      beforeAll(async () => {
        ...
        const fr = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Target' });
        foreignRecordId = fr.id;
    ...
    describe('oneOne FK column in database', () => {
      ...
      beforeAll(async () => {
        ...
        const fr = await createRecord(foreignTableId, { [foreignTitleFieldId]: 'Partner' });
        foreignRecordId = fr.id;
