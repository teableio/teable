...
describe('duplicateTable (e2e)', () => {
  ...
  it('duplicates same-base cross-table link and lookup values with records', async () => {
    // Link tables take the hydrate path (not INSERT…SELECT). External foreign
    // record ids must stay stable; lookup values must resolve after remap.
    const cleanupTableIds: string[] = [];
    ...
    try {
      ...
      // Hydrate path mints new record ids (unlike bulk INSERT…SELECT).
      const duplicatedIds = new Set(duplicatedRecords.map((record) => record.id));
      expect(duplicatedIds.has(sourceOrder1.id)).toBe(false);
  ...
  it('two-way manyOne cross-table duplicate becomes one-way and leaves foreign symmetric intact', async () => {
    ...
    try {
      ...
      for (const duplicatedRecord of duplicatedRecords) {
        expect(collectIds(f1Symmetric)).not.toContain(duplicatedRecord.id);
        expect(collectIds(f2Symmetric)).not.toContain(duplicatedRecord.id);
      }
    } finally {
      for (const tableId of cleanupTableIds.reverse()) {
        try {
