...
export const createConvergenceHarness = async (profile: ExecutionProfile) => {
  ...
  // Outbox drains run CPU-heavy work between HTTP requests. Keep the test
  // connection alive until close() rather than racing Node's 5s idle timeout
  // against fetch reusing it after a long drain on a slower runner
  // (`fetch failed: other side closed`). Same fix as computed-stage-budget.
