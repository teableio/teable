...
describe('undo-redo/createField field matrix (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    env = await createFieldUndoRedoMatrixEnv(ctx, 'Undo Matrix Create Field');
  });

  test.each(fieldMatrixCases)('create %s undo/redo', async (fieldCase) => {
    sequence += 1;
