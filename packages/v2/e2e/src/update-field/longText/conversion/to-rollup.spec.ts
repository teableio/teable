/**
 * E2E tests for converting longText field to rollup.
 *
...
describe('update-field: longText → rollup conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const foreignTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'LongText to Rollup Foreign',
      fields: [
        {
          type: 'singleLineText',
  ...
  test('should convert longText to rollup and clear data', async () => {
    ...
    const hostRecord = await ctx.createRecord(hostTableId, {
      ...
      [fieldId]: 'Some long text content',
    });
  ...
  test('should handle null values', async () => {
    const fieldId = await createLongTextField('Null LongText Field');
    const hostRecord = await ctx.createRecord(hostTableId, {
