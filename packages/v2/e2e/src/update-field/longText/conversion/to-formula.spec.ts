/**
 * E2E tests for converting longText field to formula.
 *
...
describe('update-field: longText → formula conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'LongText to Formula Conversion',
  ...
  test('should reject conversion to formula when expression is invalid', async () => {
    const fieldId = await createLongTextField('Invalid Expression Field');
