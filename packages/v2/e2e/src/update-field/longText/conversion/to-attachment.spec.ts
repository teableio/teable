/**
 * E2E tests for converting longText field to Attachment.
 *
 * Conversion behavior:
...
describe('update-field: longText → attachment conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'LongText to Attachment Conversion',
  ...
  test('should clear data (incompatible types)', async () => {
    // Setup: Create longText with values
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'longText', id: fieldId, name: 'Long Text Field' },
    });
    const r1 = await ctx.createRecord(tableId, { [fieldId]: 'some text value' });
    const r2 = await ctx.createRecord(tableId, {
      [fieldId]: 'another long text with\nmultiple lines',
    });
    ...
    // Assert: Values become null (text cannot be converted to attachment)
  ...
  test('should handle already null values', async () => {
    ...
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'longText', id: fieldId, name: 'Null Long Text' },
    });
