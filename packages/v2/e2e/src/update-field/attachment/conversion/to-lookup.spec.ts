/**
 * E2E tests for converting attachment field to lookup.
 */
...
describe('update-field: attachment → lookup conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const hostTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Attachment to Lookup Host',
    ...
    const foreignTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Attachment to Lookup Foreign',
  ...
  test('should convert to lookup and recompute values from linked records', async () => {
    ...
    const rec = records.find((r) => r.id === hostRecord.id);
    const lookupValue = rec?.fields[fieldId];
    expect(lookupValue).toEqual(['Acme Inc']);
