/**
 * E2E tests for converting date field to User.
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: date → user conversion', () => {
  ...
  test('should convert non-null date values to null (incompatible conversion)', async () => {
    const fieldId = await createDateField('Date Field');
    const r1 = await ctx.createRecord(tableId, { [fieldId]: '2024-01-15T10:30:00.000Z' });
    const r2 = await ctx.createRecord(tableId, { [fieldId]: '2024-01-16T00:00:00.000Z' });
