/**
 * E2E tests for converting Date field to Number.
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: date → number conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Date to Number Conversion',
  ...
  test('should convert date to Unix timestamp milliseconds', async () => {
    ...
    expect(rec1?.fields[fieldId]).toBe(1704067200000);
  ...
  test('should preserve precision to milliseconds', async () => {
    const fieldId = await createDateField('Millisecond Field');
    const r1 = await ctx.createRecord(tableId, { [fieldId]: '2024-01-15T10:30:45.123Z' });
    ...
    const records = await ctx.listRecords(tableId);
    const rec1 = records.find((r) => r.id === r1.id);
    expect(rec1?.fields[fieldId]).toBe(1705314645123);
