/**
 * E2E tests for converting Checkbox field to Attachment.
 *
 * Conversion behavior:
 * - All values become null (incompatible conversion)
 * - Checkbox values (true/null) cannot represent attachments
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: checkbox → attachment conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to Attachment Conversion',
  ...
  test('should clear data (incompatible types)', async () => {
    ...
    // Assert: Value becomes null (checkbox cannot represent an attachment)
  ...
  test('should handle already null values', async () => {
    // Setup: Create checkbox field with null
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
