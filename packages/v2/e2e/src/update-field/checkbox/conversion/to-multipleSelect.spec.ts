...
describe('update-field: checkbox → multipleSelect conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to multipleSelect conversion',
  ...
  test('should convert booleans to wrapped arrays with auto-generation', async () => {
    ...
    expect(records.find((r) => r.id === r2.id)?.fields[fieldId]).toEqual(['true']);
