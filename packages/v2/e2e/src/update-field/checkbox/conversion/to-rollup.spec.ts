/**
 * E2E tests for converting checkbox field to rollup.
 *
...
describe('update-field: checkbox → rollup conversion', () => {
  ...
  const createCheckboxField = async (name: string) => {
  ...
  beforeAll(async () => {
    ...
    const foreignTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to Rollup Foreign',
      fields: [
        {
          type: 'singleLineText',
  ...
  test('should convert checkbox to rollup and clear data', async () => {
    const fieldId = await createCheckboxField('Checkbox Field');
  ...
  test('should handle null values', async () => {
    const fieldId = await createCheckboxField('Null Checkbox Field');
    const hostRecord = await ctx.createRecord(hostTableId, {
