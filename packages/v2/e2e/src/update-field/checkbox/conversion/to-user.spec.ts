...
describe('update-field: checkbox → user conversion', () => {
  ...
  const createCheckboxField = async (name: string) => {
    ...
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'checkbox', id: fieldId, name },
    });
    return fieldId;
  };
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to User Conversion',
  ...
  test('should keep null values as null', async () => {
    const fieldId = await createCheckboxField('Nullable Checkbox Field');
