/**
 * E2E tests for converting Checkbox field to SingleLineText.
 *
 * Conversion behavior:
 * - true becomes "true"
...
describe('update-field: checkbox → singleLineText conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to SingleLineText Conversion',
  ...
  test('should convert true to "true"', async () => {
  ...
  test('should handle null values', async () => {
    ...
    // Assert: true becomes "true", null remains null
    const records = await ctx.listRecords(tableId);
    const rec1 = records.find((r) => r.id === r1.id);
