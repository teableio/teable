/**
 * E2E tests for converting checkbox field to formula.
 *
...
describe('update-field: checkbox → formula conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Checkbox to Formula Conversion',
  ...
  test('should reject conversion to formula when expression is invalid', async () => {
    const fieldId = await createCheckboxField('Invalid Expression Field');
