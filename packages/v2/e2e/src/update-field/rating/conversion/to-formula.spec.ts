/**
 * E2E tests for converting rating field to formula.
 *
...
describe('update-field: rating → formula conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Rating to Formula Conversion',
  ...
  test('should reject conversion to formula when expression is invalid', async () => {
    const fieldId = await createRatingField('Invalid Expression Field');
