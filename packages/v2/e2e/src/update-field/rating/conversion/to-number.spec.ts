/**
 * E2E tests for converting Rating field to Number.
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: rating → number conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Rating to Number Conversion',
  ...
  test('should convert ratings to numbers', async () => {
    ...
    expect(rec2?.fields[fieldId]).toBe(3);
  ...
  test('should preserve precision if any', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: {
        type: 'rating',
        id: fieldId,
        name: 'Precision Rating Field',
        options: { max: 5, icon: 'star', color: 'yellowBright' },
      },
    });
    const r1 = await ctx.createRecord(tableId, { [fieldId]: 2 });
    const r2 = await ctx.createRecord(tableId, { [fieldId]: 4 });
    ...
    expect(rec2?.fields[fieldId]).toBe(4);
