/**
 * E2E tests for converting singleSelect field to formula.
 *
...
describe('update-field: singleSelect → formula conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to Formula Conversion',
  ...
  test('should reject conversion to formula when expression is invalid', async () => {
    const fieldId = await createSingleSelectField('Invalid Expression Field');
  ...
  test('should handle null values', async () => {
    const fieldId = await createSingleSelectField('Null SingleSelect Field');
