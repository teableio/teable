/**
 * E2E tests for converting SingleSelect field to SingleLineText.
 *
 * Conversion behavior:
 * - singleSelect stores text, singleLineText is text -> value preserved as-is
...
describe('update-field: singleSelect → singleLineText conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to SingleLineText Conversion',
  ...
  test('should convert select values to text', async () => {
    // Setup: Create singleSelect field with values
  ...
  test('should handle null values', async () => {
    ...
    // Assert: null remains null, value preserved
    const records = await ctx.listRecords(tableId);
    const rec1 = records.find((r) => r.id === r1.id);
    const rec2 = records.find((r) => r.id === r2.id);
    expect(rec1?.fields[fieldId]).toBe('Red');
