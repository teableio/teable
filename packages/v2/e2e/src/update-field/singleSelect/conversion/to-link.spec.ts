...
describe('update-field: singleSelect → link conversion', () => {
  ...
  const createSingleSelectField = async (name: string) => {
    ...
    await ctx.createField({
      ...
      field: {
        ...
        options: {
          choices: [
            { name: 'Apple', color: 'redBright' },
            { name: 'Orange', color: 'orangeBright' },
          ],
        },
      },
    });
  ...
  beforeAll(async () => {
    ...
    const hostTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to Link Host',
    ...
    const foreignTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to Link Foreign',
