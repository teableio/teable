...
describe('update-field: singleSelect → lookup conversion', () => {
  ...
  const createSingleSelectField = async (name: string) => {
    ...
    await ctx.createField({
      ...
      field: {
        ...
        options: {
          choices: [
            { name: 'Open', color: 'greenBright' as const },
            { name: 'Closed', color: 'redBright' as const },
          ],
        },
      },
    });
  ...
  beforeAll(async () => {
    ...
    const hostTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to Lookup Host',
    ...
    const foreignTable = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleSelect to Lookup Foreign',
