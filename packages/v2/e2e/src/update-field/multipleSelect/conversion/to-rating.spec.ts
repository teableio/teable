/**
 * E2E tests for converting MultipleSelect field to Rating.
 *
 * Conversion behavior:
 * - Incompatible -> all values become null (arrays can't become ratings)
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: multipleSelect → rating conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'MultipleSelect to Rating Conversion',
  ...
  test('should convert to null (not compatible)', async () => {
    ...
    // Assert: Values become null (arrays can't become ratings)
