/**
 * E2E tests for converting MultipleSelect field to Number.
 *
 * Conversion behavior:
 * - Incompatible -> all values become null (arrays can't become numbers)
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: multipleSelect → number conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'MultipleSelect to Number Conversion',
  ...
  test('should convert to null (not compatible)', async () => {
    ...
    // Assert: Values become null (arrays can't become numbers)
