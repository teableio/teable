...
describe('update-field: multipleSelect → user conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'MultipleSelect to User Conversion',
  ...
  test('should convert non-null values to null', async () => {
    ...
    const r2 = await ctx.createRecord(tableId, { [fieldId]: ['Alpha', 'Beta'] });
  ...
  test('should keep null values as null', async () => {
    const fieldId = await createMultipleSelectField('Nullable MultipleSelect Field');
