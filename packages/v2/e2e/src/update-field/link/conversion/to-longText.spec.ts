...
describe('update-field: link to longText conversion', () => {
  ...
  test('should convert many-one one-way link to longText', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2lt-mo-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2lt-mo-b'),
  ...
  test('should convert one-many one-way link to longText', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2lt-om-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2lt-om-b'),
      fields: [{ type: 'singleLineText', name: 'Title', isPrimary: true }],
    });
