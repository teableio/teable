...
describe('update-field: link to singleSelect conversion', () => {
  ...
  test('should convert many-one one-way link to singleSelect with immediate generated choices', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-mo-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-mo-b'),
  ...
  test('should convert one-many one-way link to singleSelect with immediate generated choices', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-b'),
      fields: [{ type: 'singleLineText', name: 'Title', isPrimary: true }],
    });
  ...
  test('should join titles when converting one-many one-way link with multiple values to singleSelect', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-join-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-join-b'),
      fields: [{ type: 'singleLineText', name: 'Title', isPrimary: true }],
    });
  ...
  test('should keep null when converting empty one-many one-way link to singleSelect', async () => {
    const tableA = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-null-a'),
      fields: [{ type: 'singleLineText', name: 'Name', isPrimary: true }],
    });
    const tableB = await ctx.createTable({
      baseId: ctx.baseId,
      name: nextName('l2ss-om-null-b'),
      fields: [{ type: 'singleLineText', name: 'Title', isPrimary: true }],
    });
