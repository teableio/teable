...
describe('update-field: number → checkbox conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Number to Checkbox Conversion',
  ...
  test('should convert 0 to false', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'number', id: fieldId, name: 'Zero Number Field' },
    });
  ...
  test('should convert non-zero to true', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'number', id: fieldId, name: 'Non-zero Number Field' },
    });
    const r1 = await ctx.createRecord(tableId, { [fieldId]: 1 });
    const r2 = await ctx.createRecord(tableId, { [fieldId]: 100 });
    const r3 = await ctx.createRecord(tableId, { [fieldId]: -5 });
    const r4 = await ctx.createRecord(tableId, { [fieldId]: 0.5 });
  ...
  test('should convert negative numbers to true', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'number', id: fieldId, name: 'Negative Number Field' },
    });
    const r1 = await ctx.createRecord(tableId, { [fieldId]: -1 });
    const r2 = await ctx.createRecord(tableId, { [fieldId]: -100 });
