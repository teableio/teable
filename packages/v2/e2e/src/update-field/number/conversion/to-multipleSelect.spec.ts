...
describe('update-field: number → multipleSelect conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'Number to multipleSelect conversion',
  ...
  test('should convert numbers to wrapped array options with auto-generation', async () => {
    ...
    expect(records.find((r) => r.id === r2.id)?.fields[fieldId]).toEqual(['2']);
    ...
    const refreshedField = refreshedTable.fields.find((f) => f.id === fieldId) as
      | SelectFieldLike
      | undefined;
    expect(getChoiceNames(refreshedField)).toEqual(['1', '2', '3']);
  ...
  test('should handle null values', async () => {
    ...
    expect(records.find((r) => r.id === r3.id)?.fields[fieldId]).toEqual(['2']);
  ...
  test('should handle decimal numbers', async () => {
    ...
    const records = await ctx.listRecords(tableId);
    expect(records.find((r) => r.id === r1.id)?.fields[fieldId]).toEqual(['1.5']);
    expect(records.find((r) => r.id === r2.id)?.fields[fieldId]).toEqual(['2.75']);
