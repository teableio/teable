...
describe('update-field: singleLineText → rating conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleLineText to Rating Conversion',
  ...
  test('should convert numeric strings to ratings', async () => {
    // Setup: Create singleLineText with numeric values
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'singleLineText', id: fieldId, name: 'Rating Field' },
    });
  ...
  test('should convert non-numeric strings to null', async () => {
    // Setup: Create singleLineText with non-numeric values
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'singleLineText', id: fieldId, name: 'Invalid Rating' },
    });
    const r1 = await ctx.createRecord(tableId, { [fieldId]: 'hello' });
    const r2 = await ctx.createRecord(tableId, { [fieldId]: 'not a number' });
  ...
  test('should handle null values', async () => {
    ...
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'singleLineText', id: fieldId, name: 'Null Rating' },
    });
  ...
  test('should handle mixed valid and invalid values', async () => {
    ...
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
      field: { type: 'singleLineText', id: fieldId, name: 'Mixed Rating' },
    });
    ...
    const rec2 = records.find((r) => r.id === r2.id);
    const rec3 = records.find((r) => r.id === r3.id);
    expect(rec1?.fields[fieldId]).toBe(2);
    expect(rec2?.fields[fieldId]).toBeNull();
    expect(rec3?.fields[fieldId]).toBe(5);
