/**
 * E2E tests for converting singleLineText field to Attachment.
 *
 * Conversion behavior:
 * - Text values become null (incompatible types)
...
describe('update-field: singleLineText → attachment conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'SingleLineText to Attachment Conversion',
  ...
  test('should clear data (incompatible types)', async () => {
    ...
    const r2 = await ctx.createRecord(tableId, { [fieldId]: 'https://example.com/image.png' });
