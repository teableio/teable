...
describe('update-field: user → singleSelect conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'user to singleSelect conversion',
  ...
  test('should convert user to select option and keep null values', async () => {
  ...
  test('should convert multiple users to a single joined option string', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
    ...
    const records = await ctx.listRecords(tableId);
    expect(records.find((r) => r.id === r1.id)?.fields[fieldId]).toBe('System, test');
