/**
 * E2E tests for converting User field to Attachment.
 */
/* eslint-disable @typescript-eslint/naming-convention */
import { afterAll, beforeAll, describe, expect, test } from 'vitest';
import { getSharedTestContext, type SharedTestContext } from '../../../shared/globalTestContext';

describe('update-field: user → attachment conversion', () => {
  ...
  beforeAll(async () => {
    ...
    const table = await ctx.createTable({
      baseId: ctx.baseId,
      name: 'user to attachment conversion',
  ...
  test('should clear data (incompatible types)', async () => {
    ...
    const r1 = await ctx.createRecord(tableId, {
      [fieldId]: { id: 'system', title: 'System', email: 'system@teable.io' },
    });
  ...
  test('should handle multiple user field', async () => {
    const fieldId = createFieldId();
    await ctx.createField({
      baseId: ctx.baseId,
      tableId,
