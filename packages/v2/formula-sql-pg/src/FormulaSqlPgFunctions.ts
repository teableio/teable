...
export class FormulaSqlPgFunctions extends FormulaSqlPgExpressionBuilder {
  ...
  private concatenate(params: SqlExpr[]): SqlExpr {
    const parts = params.map((param) => this.coerceToString(param));
  ...
  private extractDatePart(part: string, params: SqlExpr[]): SqlExpr {
    ...
    const valueSql = `EXTRACT(${part} FROM (${datetime.valueSql})::timestamptz)::int`;
    return makeExpr(
  ...
  private workday(params: SqlExpr[]): SqlExpr {
    ...
    const valueSql = `(${startDt.valueSql})::date + INTERVAL '1 day' * ${dayCount.valueSql}`;
    return makeExpr(
      guardValueSql(valueSql, errorCondition),
      'datetime',
      false,
  ...
  private workdayDiff(params: SqlExpr[]): SqlExpr {
    ...
    const valueSql = `(${endDt.valueSql})::date - (${startDt.valueSql})::date`;
    return makeExpr(
      guardValueSql(valueSql, errorCondition),
      'number',
      false,
  ...
  private datestr(params: SqlExpr[]): SqlExpr {
    ...
    if (!value) return makeExpr('NULL', 'string', false);
    const datetime = this.coerceToDatetime(value);
    const valueSql = `(${datetime.valueSql})::date::text`;
    return makeExpr(
      guardValueSql(valueSql, datetime.errorConditionSql),
      'string',
      false,
  ...
  private timestr(params: SqlExpr[]): SqlExpr {
    ...
    if (!value) return makeExpr('NULL', 'string', false);
    const datetime = this.coerceToDatetime(value);
    const valueSql = `(${datetime.valueSql})::time::text`;
    return makeExpr(
      guardValueSql(valueSql, datetime.errorConditionSql),
      'string',
      false,
  ...
  private datetimeFormat(params: SqlExpr[]): SqlExpr {
    ...
    if (!value || !format) return makeExpr('NULL', 'string', false);
    const datetime = this.coerceToDatetime(value);
    ...
    const valueSql = `TO_CHAR(${datetime.valueSql}, ${formatText.valueSql})`;
    return makeExpr(
      guardValueSql(valueSql, errorCondition),
      'string',
      false,
  ...
  private datetimeParse(params: SqlExpr[]): SqlExpr {
    ...
    const valueSql = `(CASE
      ...
      WHEN pg_input_is_valid(${textValue.valueSql}::text, 'timestamptz') THEN (${textValue.valueSql})::timestamptz
      WHEN pg_input_is_valid(${textValue.valueSql}::text, 'timestamp') THEN (${textValue.valueSql})::timestamp
      ELSE NULL
    END)`;
    const errorCondition = `(${textValue.valueSql} IS NOT NULL AND NOT (pg_input_is_valid(${textValue.valueSql}::text, 'timestamptz') OR pg_input_is_valid(${textValue.valueSql}::text, 'timestamp')))`;
  ...
  private count(params: SqlExpr[]): SqlExpr {
    ...
    const terms = params.map((param) => {
      if (param.isArray) {
        ...
        return `COALESCE(jsonb_array_length(${normalized}), 0)`;
      }
