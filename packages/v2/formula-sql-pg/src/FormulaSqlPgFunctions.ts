...
export class FormulaSqlPgFunctions extends FormulaSqlPgExpressionBuilder {
  ...
  private encodeUrlComponent(params: SqlExpr[]): SqlExpr {
    ...
    const valueSql = `encode(${text.valueSql}::bytea, 'escape')`;
    return makeExpr(
  ...
  private extractDatePart(part: string, params: SqlExpr[]): SqlExpr {
    ...
    if (value.isArray) {
      ...
      const elementValueSql = guardValueSql(
        `EXTRACT(${part} FROM ${this.applyFormulaTimeZone(elementDatetime.valueSql)})::int`,
    ...
    const valueSql = `EXTRACT(${part} FROM ${this.applyFormulaTimeZone(datetime.valueSql)})::int`;
    return makeExpr(
  ...
  private fromNow(params: SqlExpr[]): SqlExpr {
    ...
    if (!value) return makeExpr('NULL', 'number', false);
    const datetime = this.coerceToDatetime(value);
    const valueSql = `EXTRACT(EPOCH FROM (NOW() - ${datetime.valueSql}))`;
    return makeExpr(
      guardValueSql(valueSql, datetime.errorConditionSql),
      'number',
      false,
  ...
  private toNow(params: SqlExpr[]): SqlExpr {
    ...
    if (!value) return makeExpr('NULL', 'number', false);
    const datetime = this.coerceToDatetime(value);
    const valueSql = `EXTRACT(EPOCH FROM (${datetime.valueSql} - NOW()))`;
    return makeExpr(
      guardValueSql(valueSql, datetime.errorConditionSql),
      'number',
      false,
  ...
  private datetimeFormat(params: SqlExpr[]): SqlExpr {
    ...
    if (value.isArray) {
      ...
      const elementDatetime = this.coerceToDatetime(elemExpr);
      const elementValueSql = guardValueSql(
        `TO_CHAR(${this.applyFormulaTimeZone(elementDatetime.valueSql)}, ${normalizedFormatSql})`,
        elementDatetime.errorConditionSql
      );
      const valueSql = this.stringifyNormalizedJsonArrayWithElement(
        normalizedArray,
    ...
    // Handle missing format by using default, or normalize the provided format
    let normalizedFormatSql: string;
    if (!format) {
      // Default to 'YYYY-MM-DD' when format is omitted
      normalizedFormatSql = normalizeDatetimeFormatExpression(null);
    } else {
      const formatText = this.coerceToString(format);
      // Try to extract string literal for normalization
      const formatLiteral = this.getStringLiteralValue(formatText);
      if (formatLiteral !== undefined) {
        // Normalize format tokens to PostgreSQL tokens
        normalizedFormatSql = normalizeDatetimeFormatExpression(`'${formatLiteral}'`);
      } else {
        // Dynamic format - can't normalize at compile time, pass through
        normalizedFormatSql = formatText.valueSql;
      }
      ...
      const valueSql = `TO_CHAR(${this.applyFormulaTimeZone(
        datetime.valueSql
      )}, ${normalizedFormatSql})`;
      return makeExpr(
        guardValueSql(valueSql, errorCondition),
    ...
    // When format is missing, no need to check format errors
    const errorCondition = datetime.errorConditionSql;
    const errorMessage = datetime.errorMessageSql;
    const valueSql = `TO_CHAR(${this.applyFormulaTimeZone(
      datetime.valueSql
    )}, ${normalizedFormatSql})`;
    return makeExpr(
