...
const compileProjection = (translator: FormulaSqlPgTranslator) => {
  if (process.env.FORMULA_COMPILER_WORKLOAD === 'single-json') {
    ...
    return translator
      .renderExpression(expression, (raw) =>
  ...
  return translator
    .renderExpressions(expressions, (raw) =>
  ...
  it.each(['UTC', 'Asia/Shanghai', 'America/New_York'])(
    ...
    (zone) => {
      ...
      try {
        ...
        expect(compileProjection(baseline)).toBe(projection);
      } finally {
        parse.mockRestore();
      }
    }
  ...
  () => {
    ...
    const run = (baseline: boolean) => {
      ...
      try {
        ...
        for (const translator of translators) compileProjection(translator);
