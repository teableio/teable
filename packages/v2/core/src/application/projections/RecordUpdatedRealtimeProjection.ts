...
export class RecordUpdatedRealtimeProjection implements IEventHandler<RecordUpdated> {
  ...
  async handle(
    ...
    return safeTry(async function* () {
      ...
      // Build initial snapshot from changes (for ensure - handles missing docs)
      ...
      // Ensure document exists (handles historical records without ShareDB docs)
      yield* (await realtimeEngine.ensure(context, docId, snapshot)).safeUnwrap();

      // Apply incremental changes for each field
      // Using applyChange ensures proper version management without jumps
      for (const change of event.changes) {
        yield* (
