...
@ProjectionHandler(ViewOrderUpdated)
@injectable()
export class ViewOrderUpdatedRealtimeProjection implements IEventHandler<ViewOrderUpdated> {
  constructor(
  ...
  async handle(
    ...
    return scheduleRealtimeProjection(
      context,
      ViewOrderUpdatedRealtimeProjection.name,
      (context, scope) =>
          ...
          yield* (
            await realtimeEngine.applyChange(
              ...
              withPersistedViewAuditChanges(
                ...
                {
                  type: 'set',
                  path: ['views', viewIndex, 'order'],
                  value: event.nextOrder.toNumber(),
                  oldValue: event.previousOrder.toNumber(),
                },
                ['views', viewIndex]
              )
            )
