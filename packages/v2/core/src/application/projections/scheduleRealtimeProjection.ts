...
const realtimeProjectionScopes = new WeakMap<IExecutionContext, RealtimeProjectionScope>();

export const getRealtimeProjectionScope = (context: IExecutionContext): RealtimeProjectionScope => {
  const existing = realtimeProjectionScopes.get(context);
  if (existing) {
    return existing;
  }

  const scope: RealtimeProjectionScope = {
    tableSnapshotCache: createRealtimeTableSnapshotCache(),
    viewColumnMetaRealtimePendingKeys: new Set<string>(),
  };
  realtimeProjectionScopes.set(context, scope);
  return scope;
};
...
export const scheduleRealtimeProjection = (
  ...
  const projectionScope = getRealtimeProjectionScope(context);
  const run = async () => {
