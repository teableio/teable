...
import { bindRealtimeTableSnapshotCache } from './RealtimeTableSnapshotCache';
...
export const scheduleRealtimeProjection = (
  ...
  bindRealtimeTableSnapshotCache(context, backgroundContext);
  const run = async () => {
    ...
    const execute = async () => {
      try {
        const result = await task(backgroundContext);
        if (result.isErr()) {
