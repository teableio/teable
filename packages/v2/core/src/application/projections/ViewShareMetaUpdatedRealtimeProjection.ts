...
@ProjectionHandler(ViewShareMetaUpdated)
@injectable()
export class ViewShareMetaUpdatedRealtimeProjection implements IEventHandler<ViewShareMetaUpdated> {
  constructor(
  ...
  async handle(
    ...
    return scheduleRealtimeProjection(
      context,
      ViewShareMetaUpdatedRealtimeProjection.name,
      (context, scope) =>
        safeTry(async function* () {
          const snapshot = yield* (
            await loadRealtimeTableSnapshot(context, {
              ...
              isSnapshotUsable: (candidate) => {
                ...
                return JSON.stringify(view?.shareMeta) === JSON.stringify(event.nextShareMeta);
              },
            })
          ).safeUnwrap();
            ...
            await realtimeEngine.applyChange(
              ...
              withPersistedViewAuditChanges(
                ...
                [
                  {
                    type: 'set',
                    path: ['views', viewIndex, 'shareMeta'],
                    value: viewDto.shareMeta,
                  },
                ],
                ['views', viewIndex]
              )
          ...
          return realtimeEngine.applyChange(
            ...
            withPersistedViewAuditChanges(viewDto, [
              { type: 'set', path: ['shareMeta'], value: viewDto.shareMeta },
            ]),
