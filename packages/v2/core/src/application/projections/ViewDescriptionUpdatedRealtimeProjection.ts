...
@injectable()
export class ViewDescriptionUpdatedRealtimeProjection
  implements IEventHandler<ViewDescriptionUpdated>
{
  constructor(
  ...
  async handle(
    ...
    return scheduleRealtimeProjection(
      context,
      ViewDescriptionUpdatedRealtimeProjection.name,
      (context, scope) =>
            ...
            await loadRealtimeTableSnapshot(context, {
              ...
              isSnapshotUsable: (candidate) =>
                candidate.views.some(
                  (view) =>
                    view.id === event.viewId.toString() &&
                    view.description === event.nextDescription
                ),
            })
          ).safeUnwrap();
