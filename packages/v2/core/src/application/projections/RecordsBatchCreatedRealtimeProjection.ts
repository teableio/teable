...
export class RecordsBatchCreatedRealtimeProjection implements IEventHandler<RecordsBatchCreated> {
  ...
  async handle(
    ...
    return safeTry(async function* () {
      ...
      // safeTry(async function*() { ... return ok(undefined); }) infers
      // ResultAsync<undefined, _>; type the array to match the actual shape.
      const tasks: Array<() => ResultAsync<undefined, DomainError>> = [];
