...
- `NoopButtonClickWorkflowService.ts` - Role: IButtonClickWorkflowService default; Purpose: no workflow run. Table Button field only — not a host-hook template.
- `NoopComputedOutboxAdmin.ts` - Role: IComputedOutboxAdmin default; Purpose: empty reads, not-implemented writes.
...
- `NoopTableRecordQueryRepository.ts` - Role: ITableRecordQueryRepository default; Purpose: empty record reads and zero counts.
...
- `NoopViewPluginRepository.ts` - Role: IViewPluginRepository default; Purpose: not-configured plugin views. Table plugin views only — not a host-hook template.
