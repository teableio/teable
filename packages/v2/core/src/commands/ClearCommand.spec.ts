...
describe('ClearCommand', () => {
  ...
  it('includes projection when provided', () => {
    const commandResult = ClearCommand.create({
      ...
      projection: ['fld1111111111111111', 'fld2222222222222222'],
    });

    const command = commandResult._unsafeUnwrap();
    expect(command.projection).toEqual(['fld1111111111111111', 'fld2222222222222222']);
  });
  ...
  it('includes groupBy when provided', () => {
    const commandResult = ClearCommand.create({
      ...
      groupBy: [{ fieldId: 'fld3333333333333333', order: 'desc' }],
    });
  ...
  it('rejects columns type with wrong ranges format', () => {
    const commandResult = ClearCommand.create({
      tableId,
      viewId,
      ranges: [
  ...
  it('rejects rows type with wrong ranges format', () => {
    const commandResult = ClearCommand.create({
      tableId,
      viewId,
      ranges: [
