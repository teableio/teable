...
export class CreateViewResult {
  ...
  static create(
    ...
    return new CreateViewResult(table, viewId, [...events]);
  }
}

@CommandHandler(CreateViewCommand)
@injectable()
export class CreateViewHandler implements ICommandHandler<CreateViewCommand, CreateViewResult> {
  constructor(
  ...
  async handle(
    ...
    return safeTry<CreateViewResult, DomainError>(async function* () {
      ...
      const prepared = yield* await handler.viewPluginCreationService.prepare(
        context,
        table,
        command.view
      );
      const createResult = yield* table.createView(prepared.input);
      const { view, updateResult: viewUpdateResult } = createResult;
      ...
      return ok(CreateViewResult.create(updateResult.table, view.id(), updateResult.events));
    });
  }
}
