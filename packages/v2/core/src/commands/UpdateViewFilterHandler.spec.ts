...
import { UpdateViewFilterHandler } from './UpdateViewFilterHandler';
...
const createHandler = (table: Table, plugins: IViewOperationPlugin[] = []) => {
  ...
  return {
    handler: new UpdateViewFilterHandler(
      tableRepository,
...
describe('UpdateViewFilterCommand', () => {
  it('validates identifiers and the public filter contract', () => {
    expect(
      UpdateViewFilterCommand.create({ tableId: 'bad', viewId: 'bad', filter: null }).isErr()
    ).toBe(true);
    const table = buildTable();
    expect(
      UpdateViewFilterCommand.create({
  ...
  it('preserves incomplete list conditions for the aggregate', () => {
    const table = buildTable();
    const filter = {
    ...
    const command = UpdateViewFilterCommand.create({
      tableId: table.id().toString(),
      viewId: table.views()[0]!.id().toString(),
      filter,
    })._unsafeUnwrap();

    expect(command.filter).toEqual(filter);
  });
});

describe('UpdateViewFilterHandler', () => {
  it('orchestrates aggregate mutation, plugin policy, persistence, and v2 history', async () => {
    ...
    expect(prepare).toHaveBeenCalledWith(
      expect.objectContaining({
        kind: 'update',
        payload: expect.objectContaining({ patch: { filter } }),
      })
    );
  ...
  it('skips plugins, persistence, and history for an identical filter', async () => {
    const table = buildTable();
    const viewId = table.views()[0]!.id();
    const filter = {
      conjunction: 'and' as const,
      filterSet: [
        { fieldId: table.getFields()[0]!.id().toString(), operator: 'is' as const, value: 'Same' },
      ],
    };
    const current = table.updateViewFilter(viewId, filter)._unsafeUnwrap().updateResult!.table;
  ...
  it('does not persist or append history when a guard rejects', async () => {
    const table = buildTable();
    const seeded = table
      .updateViewFilter(table.views()[0]!.id(), {
        conjunction: 'and',
        filterSet: [
          {
            fieldId: table.getFields()[0]!.id().toString(),
    ...
    const setup = createHandler(seeded, [
      {
        name: 'reject',
        supports: () => true,
    ...
    const command = UpdateViewFilterCommand.create({
      tableId: seeded.id().toString(),
      viewId: seeded.views()[0]!.id().toString(),
      filter: null,
    })._unsafeUnwrap();
