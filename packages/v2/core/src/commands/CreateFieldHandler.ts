...
import { consumePreloadedTableFromExecutionContext } from '../application/services/PreloadedTableContext';
...
export class CreateFieldHandler implements ICommandHandler<CreateFieldCommand, CreateFieldResult> {
  ...
  async handle(
    ...
    return safeTry<CreateFieldResult, DomainError>(async function* () {
      ...
      const preloadedTable = consumePreloadedTableFromExecutionContext(context, {
        baseId: command.baseId,
        tableId: command.tableId,
      });
      let table = preloadedTable;
      if (!table) {
