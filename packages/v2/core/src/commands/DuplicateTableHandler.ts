  ...
  implements ICommandHandler<DuplicateTableCommand, DuplicateTableResult>
{
  ...
  async handle(
    ...
    return safeTry<DuplicateTableResult, DomainError>(async function* () {
      ...
      // Prefer server-side INSERT…SELECT for main rows + external link storage
      // (T6153 / T6156). Self-link tables still hydrate (record-id remaps).
      const usePhysicalRowCopy =
      ...
      if (command.includeRecords && !usePhysicalRowCopy) {
      ...
      const duplicateResult = await handler.unitOfWork.withTransaction(
        ...
        async (dataTransactionContext) =>
          safeTry<void, DomainError>(async function* () {
            ...
            if (usePhysicalRowCopy) {
              const plan = yield* buildPhysicalTableDuplicatePlan({
                sourceTable,
      ...
      const events = usePhysicalRowCopy
