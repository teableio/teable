...
import { UpdateViewGroupHandler } from './UpdateViewGroupHandler';
...
const createHandler = (table: Table, plugins: IViewOperationPlugin[] = []) => {
  ...
  return {
    handler: new UpdateViewGroupHandler(
      tableRepository,
...
describe('UpdateViewGroupCommand', () => {
  it('validates identifiers, directions, empty, and null groups', () => {
    expect(
      UpdateViewGroupCommand.create({ tableId: 'bad', viewId: 'bad', group: null }).isErr()
    ).toBe(true);
    const table = buildTable();
    const ids = { tableId: table.id().toString(), viewId: table.views()[0]!.id().toString() };
    expect(UpdateViewGroupCommand.create({ ...ids, group: null }).isOk()).toBe(true);
    expect(UpdateViewGroupCommand.create({ ...ids, group: [] }).isOk()).toBe(true);
    expect(
      UpdateViewGroupCommand.create({
        ...ids,
        group: [{ fieldId: table.getFields()[0]!.id().toString(), order: 'up' }],
      }).isErr()
    ).toBe(true);
  });
});

describe('UpdateViewGroupHandler', () => {
  it('orchestrates aggregate mutation, plugin policy, persistence, and v2 history', async () => {
    const table = buildTable();
    const group = [{ fieldId: table.getFields()[0]!.id().toString(), order: 'desc' as const }];
    ...
    const command = UpdateViewGroupCommand.create({
    ...
    expect(prepare).toHaveBeenCalledWith(
      expect.objectContaining({
        kind: 'update',
        payload: expect.objectContaining({ patch: { group } }),
      })
    );
  ...
  it('skips plugins, persistence, and history for an identical group', async () => {
    const table = buildTable();
    const viewId = table.views()[0]!.id();
    const group = [{ fieldId: table.getFields()[0]!.id().toString(), order: 'asc' as const }];
    const current = table.updateViewGroup(viewId, group)._unsafeUnwrap().updateResult!.table;
