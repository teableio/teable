...
export class UpdateViewLockedResult {
  private constructor(
  ...
  static create(params: {
    ...
    return new UpdateViewLockedResult(
      params.table,
      params.viewId,
...
@CommandHandler(UpdateViewLockedCommand)
@injectable()
export class UpdateViewLockedHandler
  implements ICommandHandler<UpdateViewLockedCommand, UpdateViewLockedResult>
{
  constructor(
  ...
  async handle(
    ...
    return safeTry<UpdateViewLockedResult, DomainError>(async function* () {
      ...
      const lockedResult = yield* table.updateViewLocked(command.viewId, command.isLocked);
      ...
      const pluginExecution = yield* await handler.viewOperationPluginRunner.prepare({
        ...
        payload: {
          ...
          patch: { isLocked: command.isLocked },
        },
      ...
      return ok(
        UpdateViewLockedResult.create({
          table: updateResult.table,
          viewId: command.viewId,
          previousIsLocked: lockedResult.previousIsLocked,
