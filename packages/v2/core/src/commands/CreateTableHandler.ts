  ...
  async handle(
    ...
    return safeTry<CreateTableResult, DomainError>(async function* () {
      ...
      const tablePluginExecution = yield* await handler.tableOperationPluginRunner.prepare({
        kind: TableOperationKind.create,
      ...
      context.pendingTableDataInitialization = { tableIds: [persistedTable.id().toString()] };
              ...
              safeTry<{ sideEffectEvents: ReadonlyArray<IDomainEvent> }, DomainError>(
                async function* () {
                  ...
                  const sideEffectResult =
                    yield* await handler.fieldCreationSideEffectService.execute(
                      dataTransactionContext,
                      {
                        table,
                        fields: tableFields,
