...
export class DisableViewShareResult {
  private constructor(
  ...
  static create(params: {
    ...
    return new DisableViewShareResult(params.table, params.viewId, params.previousShareId, [
      ...params.events,
    ]);
  }
}

@CommandHandler(DisableViewShareCommand)
@injectable()
export class DisableViewShareHandler
  implements ICommandHandler<DisableViewShareCommand, DisableViewShareResult>
{
  constructor(
  ...
  async handle(
    ...
    command: DisableViewShareCommand
  ): Promise<Result<DisableViewShareResult, DomainError>> {
    const handler = this;
    return safeTry<DisableViewShareResult, DomainError>(async function* () {
      ...
      const shareResult = yield* table.disableViewShare(command.viewId);
      ...
      return ok(
        DisableViewShareResult.create({
          table: update.table,
          viewId: command.viewId,
          previousShareId: shareResult.previousShareId,
