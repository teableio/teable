...
describe('ListTableRecordsHandler', () => {
  ...
  // T7334: the count decision is `includeTotal === true` (both in the find options this
  // handler builds and in the repository), so what observability reports must be that
  // same decision - "not false" reported a total for requests that never counted.
  it('reports the count decision to observability exactly as it queries', async () => {
    const table = buildTable();
    const tableRepository = new MemoryTableRepository();
    await tableRepository.insert(createContext(), table);

    const captured: { includeTotal?: boolean } = {};
    const recordQueryRepo: ITableRecordQueryRepository = {
      find: async (_context, _table, _spec, options) => {
        captured.includeTotal = options?.includeTotal;
        return ok({ records: [{ id: 'rec1', fields: {}, version: 1 }], total: 1 });
      },
      findOne: async () => err(domainError.notFound({ message: 'Not found' })),
      async *findStream() {},
    };
    const events: TableQueryObservabilityEvent[] = [];
    const observability: ITableQueryObservability = {
      recordRequest: (event) => events.push(event),
      recordError: () => undefined,
      recordSearchFallback: () => undefined,
      recordSearchValidation: () => undefined,
    };
    ...
    const handleAndReport = async (includeTotal?: boolean) => {
      ...
      expect(result.isOk()).toBe(true);
      return events.at(-1)?.includeTotal;
    };

    expect(await handleAndReport()).toBe(false);
    expect(captured.includeTotal).toBe(false);

    expect(await handleAndReport(true)).toBe(true);
    expect(captured.includeTotal).toBe(true);
  });
