...
export class TableUpdateViewDescriptionSpec<
  V extends ITableSpecVisitor = ITableSpecVisitor,
> extends MutateOnlySpec<Table, V> {
  private constructor(
    private readonly viewIdValue: ViewId,
    private readonly previousDescriptionValue: string | undefined,
  ...
  static create(
    ...
    return new TableUpdateViewDescriptionSpec(viewId, previousDescription, nextDescription);
  }
  ...
  mutate(table: Table): Result<Table, DomainError> {
    ...
    const nextPropertiesResult = targetResult.value
      .properties()
      .withDescription(this.nextDescriptionValue);
  ...
  accept(visitor: V): Result<void, DomainError> {
    return visitor.visitTableUpdateViewDescription(this).map(() => undefined);
  }
}
