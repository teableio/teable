...
export class TableUpdateViewShareIdSpec<
  V extends ITableSpecVisitor = ITableSpecVisitor,
> extends MutateOnlySpec<Table, V> {
  private constructor(
    private readonly viewIdValue: ViewId,
    private readonly previousShareIdValue: string | undefined,
    private readonly nextShareIdValue: string
  ) {
    super();
  }
  ...
  static create(
    ...
    return new TableUpdateViewShareIdSpec(viewId, previousShareId, nextShareId);
  }
  ...
  previousShareId(): string | undefined {
    return this.previousShareIdValue;
  }

  nextShareId(): string {
    return this.nextShareIdValue;
  }
  ...
  mutate(table: Table): Result<Table, DomainError> {
    ...
    const nextPropertiesResult = targetResult.value.properties().withShareId(this.nextShareIdValue);
  ...
  accept(visitor: V): Result<void, DomainError> {
    return visitor.visitTableUpdateViewShareId(this).map(() => undefined);
  }
}
