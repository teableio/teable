...
describe('Table.createRecordInputSchema', () => {
  ...
  describe('SingleSelectField', () => {
    ...
    it('generates schema for optional singleSelect with options', () => {
      ...
      // Invalid values - unknown option ID
      expect(schema.safeParse({ [fieldId]: 'unknown' }).success).toBe(false);
      expect(schema.safeParse({ [fieldId]: 'Option 1' }).success).toBe(false); // name, not id
      expect(schema.safeParse({ [fieldId]: 123 }).success).toBe(false);
    });
    ...
    it('generates schema for singleSelect without options (accepts any string)', () => {
      const result = buildTableWith((b) => {
        b.field()
          .singleSelect()
      ...
      // No options, accepts any string
      expect(schema.safeParse({ [fieldId]: 'anything' }).success).toBe(true);
      expect(schema.safeParse({ [fieldId]: null }).success).toBe(true);
      expect(schema.safeParse({ [fieldId]: 123 }).success).toBe(false);
    });
  });
  ...
  describe('MultipleSelectField', () => {
    ...
    it('generates schema for multipleSelect without options', () => {
      ...
      // No options, accepts any string array
      expect(schema.safeParse({ [fieldId]: ['any', 'strings'] }).success).toBe(true);
      expect(schema.safeParse({ [fieldId]: null }).success).toBe(true);
    });
  });
  ...
  describe('DateField', () => {
    it('generates schema for optional date field', () => {
      ...
      // Invalid values
      expect(schema.safeParse({ [fieldId]: '2024-01-15' }).success).toBe(false); // not ISO datetime
