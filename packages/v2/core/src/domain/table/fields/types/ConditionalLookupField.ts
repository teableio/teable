 ...
 * - Always multiple cell values (lookup can return multiple values from filtered records)
...
export class ConditionalLookupField extends Field implements ForeignTableRelatedField {
   ...
   * ConditionalLookup fields are always multiple (can have values from multiple filtered records).
   */
  ...
  fieldValueType(): Result<FieldValueType, DomainError> {
    return this.cellValueType().map((cellValueType) => ({
      cellValueType,
