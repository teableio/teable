...
export class FieldToSpecVisitor extends AbstractFieldVisitor<ICellValueSpec> {
  ...
  visitCheckboxField(field: CheckboxField): Result<ICellValueSpec, DomainError> {
    ...
    if (typeof this.value === 'boolean') {
      // In checkbox field, false is stored as null
      return ok(
        new SetCheckboxValueSpec(field.id(), CellValue.fromValidated(this.value ? true : null))
      );
    }
    ...
    if (this.typecast) {
      ...
      // truthy → true, falsy → null
      const finalValue = this.value ? true : null;
  ...
  visitDateField(field: DateField): Result<ICellValueSpec, DomainError> {
    ...
    if (typeof this.value === 'string') {
      ...
      if (!isNaN(date.getTime())) {
        return ok(new SetDateValueSpec(field.id(), CellValue.fromValidated(date.toISOString())));
      }
    }
  ...
  visitSingleSelectField(field: SingleSelectField): Result<ICellValueSpec, DomainError> {
    ...
    // 2. Find by name (typecast mode)
    if (this.typecast) {
      const byName = options.find((opt) => opt.name().toString() === strValue);
      if (byName) {
        return ok(
          new SetSingleSelectValueSpec(field.id(), CellValue.fromValidated(byName.id().toString()))
        );
      }
  ...
  visitMultipleSelectField(field: MultipleSelectField): Result<ICellValueSpec, DomainError> {
    ...
    for (const val of values) {
      ...
      if (this.typecast) {
        ...
        if (byName) {
          result.push(byName.id().toString());
          continue;
        }
  ...
  visitUserField(field: UserField): Result<ICellValueSpec, DomainError> {
    ...
    const parsed = this.parseUserValue(this.value);
    if (parsed.valid) {
      return ok(new SetUserValueSpec(field.id(), CellValue.fromValidated(parsed.value)));
    }

    if (this.typecast) {
      // TODO: Need SetUserValueByEmailSpec for email/name lookup
      return ok(new SetUserValueSpec(field.id(), CellValue.null()));
    }
  ...
  visitButtonField(field: ButtonField): Result<ICellValueSpec, DomainError> {
    return err(
      domainError.validation({
        code: 'validation.field.no_value',
        message: `Cannot set value for button field "${field.name().toString()}"`,
        details: { fieldId: field.id().toString(), fieldType: 'button' },
      })
    );
  }
  ...
  private parseUserValue(value: unknown): { valid: boolean; value: UserItem[] | null } {
    // Standard format: [{ id: string, email?: string, title?: string }]
    if (Array.isArray(value)) {
      const valid = value.every((v) => typeof v === 'object' && v !== null && 'id' in v);
      return { valid, value: valid ? (value as UserItem[]) : null };
    }
    return { valid: false, value: null };
  }

  private parseAttachmentValue(value: unknown): { valid: boolean; value: AttachmentItem[] | null } {
    // Standard format: [{ id, name, size, mimetype, token, ... }]
    if (Array.isArray(value)) {
