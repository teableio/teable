...
describe('FieldDefaultValueVisitor', () => {
  ...
  describe('visitSingleSelectField', () => {
    it('returns option ID (not name) when default value is configured', () => {
      const options = [
      ...
      const result = field.accept(visitor);
      // Should return the option ID, not the option name
      expect(result._unsafeUnwrap()).toBe('opt1');
    });
  ...
  describe('visitMultipleSelectField', () => {
    it('returns option IDs (not names) when default value is configured', () => {
      const options = [
        SelectOption.create({ id: 'opt1', name: 'Option 1', color: 'red' })._unsafeUnwrap(),
        SelectOption.create({ id: 'opt2', name: 'Option 2', color: 'blue' })._unsafeUnwrap(),
      ...
      const result = field.accept(visitor);
      // Should return the option IDs, not the option names
      expect(result._unsafeUnwrap()).toEqual(['opt1', 'opt3']);
    });
