...
export class FieldDefaultValueVisitor extends AbstractFieldVisitor<RawDefaultValue> {
  ...
  visitSingleSelectField(field: SingleSelectField): Result<RawDefaultValue, DomainError> {
    ...
    // defaultValue.toDto() returns option NAME, but we need option ID
    ...
    // Find the option by name and return its ID
    ...
    return ok(matchingOption.id().toString());
  }
  ...
  visitMultipleSelectField(field: MultipleSelectField): Result<RawDefaultValue, DomainError> {
    ...
    // defaultValue.toDto() returns option NAMEs, but we need option IDs
    ...
    // Find the options by name and return their IDs
    const options = field.selectOptions();
    const matchingIds = namesToFind
  ...
  visitUserField(_field: UserField): Result<RawDefaultValue, DomainError> {
    // User field default values require context (current user) - handled separately
    return ok(undefined);
  }
