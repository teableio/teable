...
export abstract class FormulaFunc {
  ...
  /**
   * The function needs to perform parameter type and quantity verification during the AST tree parsing phase.
   * If the requirements of the function are not met, return an err(...) with a friendly prompt.
   * Validation principles:
   * 1. Return an error if required parameters are missing, ignore extra parameters
   * 2. Return an error for parameter types that cannot be converted or ignored
