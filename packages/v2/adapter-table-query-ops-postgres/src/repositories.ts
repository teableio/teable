        ...
        await trx
          ...
          .values(
            input.observations.map((observation) => {
              ...
              const diagnostics = snapshot.sqlDiagnostics?.map((item) =>
                item.sampled
                  ? item
                  : {
                      source: item.source,
              ...
              return {
                ...
                sql_diagnostics: diagnostics?.length ? toJsonb(diagnostics) : null,
              };
            })
          )
          .onConflict((oc) =>
            oc
              ...
              .doUpdateSet({
                ...
                END`,
                sql_diagnostics: sql`CASE
                  WHEN table_query_observation_shard.sql_diagnostics @? '$[*] ? (@.sampled == true)'
                    AND NOT coalesce(excluded.sql_diagnostics @? '$[*] ? (@.sampled == true)', false)
                  THEN table_query_observation_shard.sql_diagnostics
                  ELSE coalesce(excluded.sql_diagnostics, table_query_observation_shard.sql_diagnostics)
                END`,
