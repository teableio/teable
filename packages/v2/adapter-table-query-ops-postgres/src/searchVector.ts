...
const buildBeforeSearchSql = (input: {
  ...
  const conditions = input.fields
    .map(
      (field) =>
        String.raw`${buildFieldProjectionSql(field, 't')} ILIKE :search_probe_like_pattern ESCAPE '\\'`
    )
    .join(' OR ');
  return [
    'EXPLAIN (FORMAT JSON)',
...
const buildSubstringSearchSql = (
  ...
  return [
    ...
    String.raw`WHERE ${documentExpression} LIKE lower(:search_probe_like_pattern) ESCAPE '\\'`,
    `  AND (${exactConditions})`,
    'LIMIT 100',
  ].join('\n');
};
...
const stableHash = (input: string): string => {
  ...
  for (let i = 0; i < input.length; i += 1) {
    hash = (hash * 31 + input.charCodeAt(i)) >>> 0; // NOSONAR typescript:S7758 -- the hash is defined over UTF-16 code units; switching to code points would change persisted/compared values
  }
...
const sameSortedValues = (left: readonly string[], right: readonly string[]): boolean => {
  ...
  const sortedLeft = [...left].sort((a, b) => Number(a > b) - Number(a < b));
  const sortedRight = [...right].sort((a, b) => Number(a > b) - Number(a < b));
