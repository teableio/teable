...
describe('PhysicalTableRecordQueryBuilder', () => {
  ...
  describe('computed-like fields (selected as physical columns)', () => {
    ...
    test('selects only projected columns', () => {
      ...
      expect(sql).toMatchInlineSnapshot(
        `"select "t"."__id" as "__id", "t"."col_formula_stored" as "col_formula_stored", "t"."col_lookup_stored" as "col_lookup_stored" from "bseaaaaaaaaaaaaaaaa"."tblmmmmmmmmmmmmmmmm" as "t""`
      );
    });
  });
