...
export const createForeignKeyConstraintStatement = (
  ...
  const targetTableResolutionSql = targetTableMetaId
    ? `
        IF EXISTS (
          SELECT 1
          ...
          INTO resolved_db_table_name
          FROM table_meta
          WHERE id = ${quoteLiteral(targetTableMetaId)}
            AND deleted_time IS NULL
          LIMIT 1;

          IF resolved_db_table_name IS NOT NULL AND resolved_db_table_name <> '' THEN
            IF strpos(resolved_db_table_name, '.') > 0 THEN
              resolved_target_schema := split_part(resolved_db_table_name, '.', 1);
              resolved_target_table := substring(
                resolved_db_table_name
                from strpos(resolved_db_table_name, '.') + 1
              );
            ELSE
              resolved_target_schema := 'public';
              resolved_target_table := resolved_db_table_name;
            END IF;
          END IF;
        END IF;
      `
  ...
  return dataStatement(
    sql.raw(
      ...
      DECLARE
        ...
        resolved_db_table_name text;
      BEGIN
        ${targetTableResolutionSql}
