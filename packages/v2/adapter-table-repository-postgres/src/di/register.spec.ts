...
describe('registerV2TableRepositoryPostgresAdapter', () => {
  it('creates the correct type validation strategy for PG16 and legacy setups', async () => {
    const { createTypeValidationStrategy } = await import('./register');
    const db = createDb();
    ...
    expect(pg16Strategy).toBeInstanceOf(Pg16TypeValidationStrategy);
    expect(legacyStrategy).toBeInstanceOf(PgLegacyTypeValidationStrategy);
  });

  it('throws when the ddl config is invalid', async () => {
    const { registerV2TableRepositoryPostgresAdapter } = await import('./register');
  ...
  it('registers default schema, record, strategy, and polling dependencies', async () => {
    const { registerV2TableRepositoryPostgresAdapter } = await import('./register');
    const { registerV2RecordRepositoryPostgresAdapter, registerV2PostgresDdlAdapter } =
    ...
    expect(getInstance(container, formulaSqlPgTokens.typeValidationStrategy)).toBeInstanceOf(
      Pg16TypeValidationStrategy
    );
    expect(
