...
vi.mock('./MetaValidationContext', async () => {
  const actual =
    await vi.importActual<typeof import('./MetaValidationContext')>('./MetaValidationContext');
  return {
    ...actual,
...
describe('MetaChecker', () => {
  it('yields context loading failures from checkTable', async () => {
    const { MetaChecker } = await import('./MetaChecker');
  ...
  it('aggregates checkTableAll results and checkTables adds table context', async () => {
    const { MetaChecker, checkTableMeta, checkTableMetaWithTables } = await import('./MetaChecker');
