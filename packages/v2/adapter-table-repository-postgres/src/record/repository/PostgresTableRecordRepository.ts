...
const isDuplicateTableRecordInsert = (
        ...
        try {
          ...
          if (!options?.skipComputedUpdates) {
            computedResult = yield* await this.runComputedUpdateMany(
              ...
              [],
              {
                allowPendingTableProvisionForComputedUpdates:
                  options?.allowPendingTableProvisionForComputedUpdates,
              }
            );
          }
  ...
  private async runComputedUpdateMany(
    ...
    if (shouldExecuteInline) {
      ...
      const plannerOptions =
        changeType === 'insert' &&
        (options.allowPendingTableProvisionForComputedUpdates === true ||
          isDuplicateTableRecordInsert(context, table, changeType))
