...
describe('StoredTableRecordQueryBuilder', () => {
  describe('basic field types', () => {
    ...
    test('orders user field by title projection with ASC null-first semantics', () => {
      ...
      expect(sql).toContain(
        `jsonb_path_query_array("t"."col_user"::jsonb, '$[*].title')::text is null desc`
      );
      expect(sql).toContain(
        `jsonb_path_query_array("t"."col_user"::jsonb, '$[*].title')::text asc`
      );
    });
    ...
    test('orders createdBy field by title with ASC null-first semantics', () => {
      ...
      expect(sql).toContain(`"t"."__created_by"::jsonb ->> 'title' is null desc`);
      expect(sql).toContain(`"t"."__created_by"::jsonb ->> 'title' asc`);
    });
  });
  ...
  describe('user field reference filters', () => {
    ...
    test('orders multiple user field by titles array text with ASC null-first semantics', () => {
      ...
      expect(sql).toContain(
        `jsonb_path_query_array("t"."col_assignees"::jsonb, '$[*].title')::text is null desc`
      );
      expect(sql).toContain(
        `jsonb_path_query_array("t"."col_assignees"::jsonb, '$[*].title')::text asc`
      );
    });
  });
