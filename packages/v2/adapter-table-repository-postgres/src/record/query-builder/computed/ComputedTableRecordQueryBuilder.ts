...
export class ComputedTableRecordQueryBuilder implements ITableRecordQueryBuilder {
  ...
  ): Result<{ column: string; userLikeMode?: 'single' | 'multiple' } | null, DomainError> {
      ...
      return table
        ...
        .andThen((field) => {
          ...
          if (fieldType.equals(FieldType.createdBy())) {
            return ok({ column: '__created_by', userLikeMode: 'single' });
          }
          if (fieldType.equals(FieldType.lastModifiedBy())) {
            return ok({ column: '__last_modified_by', userLikeMode: 'single' });
          }
