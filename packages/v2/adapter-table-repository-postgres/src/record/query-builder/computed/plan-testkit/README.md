...
The `Formula PostgreSQL Plan Gate` check runs both optimizer versions with a 512 MiB PostgreSQL container limit and no additional swap (commands in `.github/scripts/ci-formula-plan-gate.sh`). Connection failures, timeouts, excessive plans, wrong results, duplicated extraction loops, server OOM, or recovery/restart fail the check. Artifacts include SQL, JSON plans, container state and PostgreSQL logs, and are attached to the failure report. Configure the check as a required branch check to enforce the merge gate.
