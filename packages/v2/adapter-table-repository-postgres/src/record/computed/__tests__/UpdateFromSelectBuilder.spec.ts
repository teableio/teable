...
describe('UpdateFromSelectBuilder', () => {
  it('builds UPDATE FROM SELECT for computed formula field', () => {
    ...
    expect(updateResult.value.sql).toMatchInlineSnapshot(
      `
      "update "bseaaaaaaaaaaaaaaaa"."tblbbbbbbbbbbbbbbbb" as "u" set "__version" = "u"."__version" + 1, "col_score" = CASE
          WHEN ("c"."col_score") IS NULL THEN NULL
          WHEN BTRIM(("c"."col_score")::text) ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
            THEN BTRIM(("c"."col_score")::text)::double precision
          ELSE NULL
        END from (select "t"."__id" as "__id", "t"."__version" as "__version", 1 as "col_score" from "bseaaaaaaaaaaaaaaaa"."tblbbbbbbbbbbbbbbbb" as "t" where "t"."__id" in (select "d"."record_id" from "tmp_computed_dirty" as "d" where "d"."table_id" = $1)) as "c" where "u"."__id" = "c"."__id" and ("u"."col_score" IS DISTINCT FROM CASE
          WHEN ("c"."col_score") IS NULL THEN NULL
          WHEN BTRIM(("c"."col_score")::text) ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
            THEN BTRIM(("c"."col_score")::text)::double precision
          ELSE NULL
    ...
    expect(updateResult.value.sql).toMatchInlineSnapshot(
      ...
      "update "bseaaaaaaaaaaaaaaaa"."tblbbbbbbbbbbbbbbbb" as "u" set "__version" = "u"."__version" + 1, "col_score" = CASE
          ...
          WHEN BTRIM(("c"."col_score")::text) ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
            THEN BTRIM(("c"."col_score")::text)::double precision
          ELSE NULL
        END from (select "t"."__id" as "__id", "t"."__version" as "__version", 1 as "col_score" from "bseaaaaaaaaaaaaaaaa"."tblbbbbbbbbbbbbbbbb" as "t" inner join "tmp_computed_dirty" as "__dirty" on "t"."__id" = "__dirty"."record_id" and "__dirty"."table_id" = $1) as "c" where "u"."__id" = "c"."__id" and ("u"."col_score" IS DISTINCT FROM CASE
          WHEN ("c"."col_score") IS NULL THEN NULL
          WHEN BTRIM(("c"."col_score")::text) ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
            THEN BTRIM(("c"."col_score")::text)::double precision
          ELSE NULL
