...
const createTestIds = (prefix: string, count: number) => {
  const ids: Record<string, ReturnType<typeof FieldId.create>> = {};
  for (let i = 0; i < count; i++) {
    const suffix = String.fromCharCode(97 + i).repeat(16); // a, b, c, ...
    ids[`${prefix}${i}`] = FieldId.create(`fld${suffix}`);
  }
  return ids;
};
