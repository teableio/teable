...
describe('ComputedActivityProjector', () => {
  ...
  it.each([
    { asyncProjection: false, combined: false },
    { asyncProjection: false, combined: true },
    { asyncProjection: true, combined: false },
    { asyncProjection: true, combined: true },
  ])(
    'persists an unregistered cross-table failure without recording a completion: %j',
    async ({ asyncProjection, combined }) => {
      projector.configureAsyncProjection({ enabled: asyncProjection });
      const taskId = 'cross-table-failure';
        ...
        fieldErrors: [
          {
            ...
            error: {
              ...
              context: { failedFieldId: FIELD_ID, tableId: TABLE_ID_B, baseId: BASE_ID },
            },
          },
        ],
      };
      const result = combined
        ? await projector.projectStageSettlement({ done })
        : await projector.onTaskDone(done);
      result._unsafeUnwrap();
      await projector.flushAllPendingActivity();
      const failed = (
        await reader.getByTableId(undefined, TABLE_ID_B, BASE_ID, { heal: false })
      )._unsafeUnwrap();
      expect(failed.fields).toEqual([
        expect.objectContaining({
          ...
          lastError: expect.objectContaining({ code: 'computed.update_failed' }),
        }),
      ]);
      expect(failed.table?.recentCompletions ?? []).toEqual([]);
      const completed = (
        await reader.getByTableId(undefined, TABLE_ID, BASE_ID, { heal: false })
      )._unsafeUnwrap();
      expect(completed.fields[0]).toMatchObject({ fieldId: FIELD_ID, status: 'idle' });
    }
  );
  ...
  it('preserves inner activity load failures under a read budget', async () => {
    await db.schema.dropTable('computed_field_activity' as never).execute();
    const result = await reader.getByTableId(undefined, TABLE_ID, BASE_ID, { budgetMs: 2000 });
    expect(result.isErr()).toBe(true);
    const error = result._unsafeUnwrapErr();
    expect(error.message).toBe('Failed to load compute activity');
    expect(error.details).toMatchObject({ tableId: TABLE_ID });
    expect(String(error.details?.error)).not.toBe('Failed to load compute activity');
  });
  ...
  it('does not treat empty reliability with persisted JSON key order as drift', async () => {
    vi.stubEnv('COMPUTED_RELIABILITY_ENABLED', 'true');
    vi.stubEnv('COMPUTED_RELIABILITY_UI_ENABLED', 'true');
    try {
      for (const statement of computedReliabilitySchemaSql
      ...
      await db
        ...
        .values({
          ...
          last_error: null,
          // Production persist order: scopeComplete first, not emptyComputeReliability() order.
        ...
        .execute();
      const repair = vi.spyOn(projector, 'reconcileTable');
      const snapshot = (await reader.getByTableId(undefined, TABLE_ID, BASE_ID))._unsafeUnwrap();
      expect(snapshot.observationState).toBe('available');
      expect(snapshot.diagnostics.activeFieldCount).toBe(0);
      expect(repair).not.toHaveBeenCalled();
    } finally {
      vi.unstubAllEnvs();
    }
  ...
  it('does not treat missing empty reliability as drift when the ledger is empty', async () => {
    vi.stubEnv('COMPUTED_RELIABILITY_ENABLED', 'true');
    vi.stubEnv('COMPUTED_RELIABILITY_UI_ENABLED', 'true');
    try {
      for (const statement of computedReliabilitySchemaSql
  ...
  it('rolls back a budgeted repair and restores the connection for the next read', async () => {
    ...
    const next = await new PostgresComputedActivityReader(db, projector).getByTableId(
      ...
      TABLE_ID,
      BASE_ID,
      { budgetMs: 5000, heal: true }
    );
    expect(next.isOk()).toBe(true);
    expect(next._unsafeUnwrap().reconciliationPerformed).toBe(true);
  });

  it('does not heal drift on a budgeted poll unless heal is requested', async () => {
    await ensureOutboxTasks(['hot-path-reference']);
    await projector.onTaskEnqueued({
      taskId: 'hot-path-reference',
      baseId: BASE_ID,
      targets: [{ fieldId, tableId }],
      metrics,
    });
    ...
    expect(repair).not.toHaveBeenCalled();
    expect(result._unsafeUnwrap().reconciliationPerformed).not.toBe(true);
    repair.mockRestore();
  });
