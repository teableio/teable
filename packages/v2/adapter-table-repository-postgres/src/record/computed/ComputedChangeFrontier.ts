...
export const recordStageValueChanges = async (
  ...
  const changeFrontier = resolveTableSql(db, CHANGE_FRONTIER_TABLE);
  ...
  if (unsupportedTables.length > 0) {
    ...
    await sql`INSERT INTO ${changeFrontier} (scope_id, kind, table_id, record_id, field_id)
      SELECT DISTINCT ${scopeId}, 'fallback', table_id, '', '' FROM ${changeFrontier}
  ...
  const stageLedger = resolveTableSql(db, 'computed_update_stage_ledger');
  ...
  const sourceEvidence = await sql<{ present: boolean }>`SELECT EXISTS (
    SELECT 1 FROM ${stageLedger} WHERE scope_id = ${scopeId}
      AND kind IN ('frontier', 'consumed')
    UNION ALL
    SELECT 1 FROM ${changeFrontier} WHERE scope_id = ${scopeId} AND kind = 'covered'
...
export const cleanupChangeFrontierOrphans = async (
  ...
  const result = await sql<{ after_scope: string | null; deleted: number | string }>`
    WITH RECURSIVE scopes AS (
      ...
      DELETE FROM ${changeFrontier} WHERE ctid IN (
        SELECT f.ctid FROM ${changeFrontier} f
