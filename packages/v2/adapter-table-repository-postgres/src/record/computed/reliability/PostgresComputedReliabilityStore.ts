...
export class PostgresComputedReliabilityStore {
    ...
    const result = await sql
      .raw<{
        ...
        computedReliabilityReadinessSql(
          resolveTableSql(this.db, 'computed_reliability_issue').compile(this.db).sql,
          resolveTableSql(this.db, 'computed_reliability_scope').compile(this.db).sql
        )
      )
      .execute(this.db);
  ...
  private async recordFailureInTransaction(
    ...
    const refs = await sql<{ table_id: string; field_id: string }>`select table_id, field_id
      from ${resolveTableSql(this.db, 'computed_task_field_ref')} where task_id = ${input.taskId}`.execute(
      this.db
    );
    const targets = [
      ...refs.rows
    ...
    await applyBudget();
    await sql`insert into ${resolveTableSql(this.db, 'computed_reliability_issue')}(id,task_id,base_id,source_table_id,error,scope_complete,failure_kind,failure_phase,error_code)
    ...
    for (let offset = 0; offset < uniqueTargets.length; offset += 1000) {
      ...
      await sql`insert into ${resolveTableSql(this.db, 'computed_reliability_scope')}(issue_id,table_id,field_id)
  ...
  async getFieldSummaries(tableId: string, alreadyReady = false) {
    ...
    const result = await sql<{
      ...
      from ${resolveTableSql(this.db, 'computed_reliability_issue')} i join ${resolveTableSql(this.db, 'computed_reliability_scope')} s on s.issue_id=i.id
  ...
  async getTableSummary(
    ...
    const unknownCandidates =
      readableFieldIds === undefined
        ? sql`union select u.id from ${resolveTableSql(this.db, 'computed_reliability_issue')} u
          where u.source_table_id=${tableId} and u.scope_complete=false
          and not exists(select 1 from ${resolveTableSql(this.db, 'computed_reliability_scope')} us where us.issue_id=u.id)`
        : sql``;
    const result = await sql<{
      unresolved: string;
      oldest: Date | null;
      ...
      from ${resolveTableSql(this.db, 'computed_reliability_issue')} i
      join (
        select s.issue_id from ${resolveTableSql(this.db, 'computed_reliability_scope')} s
  ...
  async getUnknownScopeSummary(tableId: string, alreadyReady = false) {
    ...
    const result = await sql<{
      ...
      from ${resolveTableSql(this.db, 'computed_reliability_issue')} i where i.source_table_id=${tableId}
      and i.scope_complete=false and i.status not in ('resolved','not_applicable')
      and not exists(select 1 from ${resolveTableSql(this.db, 'computed_reliability_scope')} s where s.issue_id=i.id)`.execute(
      this.db
    );
    const row = result.rows[0];
  ...
  async listIssues(input: { baseId?: string; tableId?: string; limit?: number } = {}) {
    if (!(await this.isReady())) return [];
    const result =
      await sql<ComputedReliabilityIssue>`select i.* from ${resolveTableSql(this.db, 'computed_reliability_issue')} i
      where ${input.baseId ? sql`i.base_id=${input.baseId}` : sql`true`}
      and ${input.tableId ? sql`(i.source_table_id=${input.tableId} or exists(select 1 from ${resolveTableSql(this.db, 'computed_reliability_scope')} s where s.issue_id=i.id and s.table_id=${input.tableId}))` : sql`true`}
  ...
  /**
   * Close open, fully-scoped issues whose every (table, field) is in this
   * successful compute. Unknown-scope issues and partial coverage stay open.
   * Dead-letter replay is not this path: it never markDone's.
   */
  async resolveCoveredIssues(input: {
    baseId: string;
    succeededTargets: ReadonlyArray<{ tableId: string; fieldId: string }>;
  }): Promise<number> {
    const uniqueTargets = [
      ...new Map(
        input.succeededTargets.map((target) => [`${target.tableId}:${target.fieldId}`, target])
      ).values(),
    ];
    ...
    if (!this.db.isTransaction) {
      return this.db
        .transaction()
        .execute((trx) => new PostgresComputedReliabilityStore(trx).resolveCoveredIssues(input));
    }
    ...
    const covered = sql.join(
      uniqueTargets.map((target) => sql`(${target.tableId}::text, ${target.fieldId}::text)`)
    );
    const closed = await sql<{ id: string }>`
      with covered(table_id, field_id) as (values ${covered}),
      closable as (
        select i.id
        from ${resolveTableSql(this.db, 'computed_reliability_issue')} i
        where i.base_id = ${input.baseId}
          and i.status not in ('resolved', 'not_applicable')
          and i.scope_complete = true
          and exists (
            select 1 from ${resolveTableSql(this.db, 'computed_reliability_scope')} s
      ...
      update ${resolveTableSql(this.db, 'computed_reliability_issue')} i
      set status = 'resolved',
          closed_at = now(),
      ...
      returning i.id`.execute(this.db);
    const fieldIds = [...new Set(uniqueTargets.map((target) => target.fieldId))];
    if (fieldIds.length > 0) {
      await applyBudget();
      const savepointIdentifier = '"cri_activity_idle"';
      try {
        await sql.raw(`SAVEPOINT ${savepointIdentifier}`).execute(this.db);
        await sql`
        update ${resolveTableSql(this.db, 'computed_field_activity')}
        set status = 'idle',
            last_error = case
              when last_error is not null and last_error->>'code' = 'computed.unresolved_issue'
                then null
              else last_error
            end,
            updated_at = now()
        where field_id in (${sql.join(fieldIds)})
          and active_task_count = 0
          and status = 'failed'`.execute(this.db);
        await sql.raw(`RELEASE SAVEPOINT ${savepointIdentifier}`).execute(this.db);
      } catch (error) {
        try {
          await sql.raw(`ROLLBACK TO SAVEPOINT ${savepointIdentifier}`).execute(this.db);
          await sql.raw(`RELEASE SAVEPOINT ${savepointIdentifier}`).execute(this.db);
        } catch {
          // Keep the caller's transaction usable after a missing-table abort.
        }
