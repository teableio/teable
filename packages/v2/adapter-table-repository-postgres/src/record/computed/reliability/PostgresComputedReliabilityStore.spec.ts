...
describe('durable computed failure evidence', () => {
  ...
  it('closes a fully scoped issue when success covers every field', async () => {
    await sql`create table computed_field_activity (
    ...
    await sql`insert into computed_field_activity(field_id, status, last_error, active_task_count)
      values
        ('one', 'failed', '{"code":"computed.unresolved_issue"}'::jsonb, 0),
        ('two', 'failed', '{"code":"computed.unresolved_issue"}'::jsonb, 0)`.execute(db);
    await store.recordFailure({
      ...failure(),
      targets: [
        { tableId: 'table', fieldId: 'one' },
    ...
    expect(
      await store.resolveCoveredIssues({
        baseId: 'base',
        succeededTargets: [
          { tableId: 'table', fieldId: 'one' },
          { tableId: 'table', fieldId: 'two' },
    ...
    expect(await store.getFieldSummaries('table')).toEqual([]);
    expect(
      (
        await sql<{ field_id: string; status: string; last_error: unknown }>`
        select field_id, status, last_error from computed_field_activity order by field_id`.execute(
          db
        )
      ).rows
    ).toEqual([
      { field_id: 'one', status: 'idle', last_error: null },
      { field_id: 'two', status: 'idle', last_error: null },
    ]);
  });

  it('keeps the issue open when success misses a scoped field', async () => {
    await store.recordFailure({
      ...failure(),
      targets: [
        { tableId: 'table', fieldId: 'one' },
    ...
    expect(
      await store.resolveCoveredIssues({
        baseId: 'base',
        succeededTargets: [{ tableId: 'table', fieldId: 'one' }],
      })
    ).toBe(0);
    expect((await store.listIssues())[0]?.status).toBe('open');
  });

  it('does not auto-close unknown-scope issues', async () => {
    await store.recordFailure({ ...failure(), failureKind: 'statement_timeout' });
    expect(
      await store.resolveCoveredIssues({
        baseId: 'base',
        succeededTargets: [{ tableId: 'table', fieldId: 'one' }],
