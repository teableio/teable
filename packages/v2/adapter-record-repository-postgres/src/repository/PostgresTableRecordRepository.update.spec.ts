...
describe('PostgresTableRecordRepository.update', () => {
  it('generates update SQL for a non-link field', async () => {
    ...
    const record = TableRecord.create({
      ...
      fieldValues: [
        {
          fieldId: nameFieldId,
          value: TableRecordCellValue.create('Alice')._unsafeUnwrap(),
        },
      ],
    })._unsafeUnwrap();
    ...
    const result = await repo.update({ actorId }, table, record);
  ...
  it('generates link update SQL for manyMany links', async () => {
    ...
    const record = TableRecord.create({
      ...
      fieldValues: [
        {
          fieldId: linkFieldId,
          value: TableRecordCellValue.create([
            { id: LINKED_RECORD_A },
            { id: LINKED_RECORD_B },
          ])._unsafeUnwrap(),
        },
