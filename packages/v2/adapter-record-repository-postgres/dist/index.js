import { LinkForeignTableReferenceVisitor, LinkRelationship, domainError, isDomainError, isNotFoundError, v2CoreTokens } from "@teable/v2-core";
	...
	*/
	...
	build() {
		...
		return safeTry(function* () {
			...
			return ok(this.db.selectFrom(`${tableName} as ${T$1}`).select(() => selectColumns).$call(applyLateralJoins).$if(this.limitValue !== null, (qb) => qb.limit(this.limitValue)).$if(this.offsetValue !== null, (qb) => qb.offset(this.offsetValue)));
		}.bind(this));
	}
	...
	buildLateralJoins(table, foreignTables, laterals) {
		if (laterals.size === 0) return ok((qb) => qb);
		return safeTry(function* () {
			const subqueries = [];
			...
			return ok((qb) => subqueries.reduce((q, sub) => q.innerJoinLateral(sub, (j) => j.onTrue()), qb));
		}.bind(this));
	}
	...
	*/
	getJoinCondition(linkField, _foreignTableName) {
		...
		if (relationship.equals(LinkRelationship.manyOne()) || relationship.equals(LinkRelationship.oneOne())) {
			if (foreignKeyNameResult.isOk() && foreignKeyNameResult.value !== "__id") return ok(sql`${sql.ref(`${F}.__id`)} = ${sql.ref(`${T$1}.${foreignKeyNameResult.value}`)}`);
			if (selfKeyNameResult.isOk() && selfKeyNameResult.value !== "__id") return ok(sql`${sql.ref(`${F}.${selfKeyNameResult.value}`)} = ${sql.ref(`${T$1}.__id`)}`);
		}
		...
		if (relationship.equals(LinkRelationship.manyMany())) {
			...
			if (fkHostTableNameResult.isOk() && selfKeyNameResult.isOk() && foreignKeyNameResult.isOk()) {
				...
				return ok(sql`${sql.ref(`${F}.__id`)} IN (SELECT ${sql.ref(`j.${foreignKey}`)} FROM ${sql.table(junctionTable)} AS j WHERE ${sql.ref(`j.${selfKey}`)} = ${sql.ref(`${T$1}.__id`)})`);
			}
		}
	...
	*/
	...
	build() {
		...
		return safeTry(function* () {
			...
			return ok(this.db.selectFrom(`${tableName} as ${T}`).select(() => [idColumn, ...selectColumns]).$if(this.limitValue !== null, (qb) => qb.limit(this.limitValue)).$if(this.offsetValue !== null, (qb) => qb.offset(this.offsetValue)));
		}.bind(this));
	}
...
let PostgresTableRecordQueryRepository = class PostgresTableRecordQueryRepository$1 {
	...
	async find(context, table, _spec, options) {
		return safeTry(async function* () {
			...
			try {
				return ok(mapRowsToReadModels(fieldColumns, await builtQuery.execute()));
			} catch (error) {
				return err(domainError.unexpected({ message: `Failed to load table records: ${describeError(error)}` }));
			}
		}.bind(this));
	}
};
...
//#endregion
export { ComputedFieldSelectExpressionVisitor, ComputedTableRecordQueryBuilder, FieldOutputColumnVisitor, PostgresTableRecordQueryRepository, SqlAggregate, StoredFieldSelectVisitor, StoredTableRecordQueryBuilder, TableRecordQueryBuilderManager, registerV2RecordRepositoryPostgresAdapter, v2RecordRepositoryPostgresTokens };
