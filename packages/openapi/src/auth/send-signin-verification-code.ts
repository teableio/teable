...
export const sendSigninVerificationCodeRoute: RouteConfig = registerRoute({
  ...
  description:
    'Send a one-time sign-in verification code to a registered email. No captcha is required.',
  request: {
    body: {
      content: {
        'application/json': {
...
export const sendSigninVerificationCode = (email: string) =>
  axios.post<ISendSigninVerificationCodeVo>(SEND_SIGNIN_VERIFICATION_CODE, { email });
