...
export const tableFullVoSchema = z
  .object({
    ...
    dbTableName: z
      ...
      .meta({
        description:
          'Table name in backend database. Limitation: 1-63 characters, start with letter, can only contain letters, numbers and underscore, case insensitive, cannot be duplicated with existing db table name in the project.',
      }),
    ...
    fields: fieldVoSchema.array().meta({
      description: 'The fields of the table.',
    }),
    views: viewVoSchema.array().meta({
    ...
    defaultViewId: z.string().startsWith(IdPrefix.View).optional().meta({
...
export const tableRoSchema = tableFullVoSchema
  ...
  .extend({
    ...
    fields: createFieldRoSchema.array().optional().meta({
      description:
        'The fields of the table. If it is empty, 3 fields include SingleLineText, Number, SingleSelect will be generated by default.',
    }),
    views: viewRoSchema.array().optional().meta({
      description:
        'The views of the table. If it is empty, a grid view will be generated by default.',
    }),
    records: createRecordsRoSchema.shape.records.optional().meta({
      description: 'The record data of the table. If it is empty, no records will be created.',
    }),
  })
  .meta({
...
export const tableListVoSchema = tableVoSchema.array().meta({
...
export const CreateTableRoute: RouteConfig = registerRoute({
  ...
  description:
    'Create a new table in the specified project with customizable fields, views, and initial records. Default configurations will be applied if not specified.',
  request: {
    params: z.object({
      baseId: z.string(),
    }),
