...
export const updateBasePersonalOrderRoute: RouteConfig = registerRoute({
  ...
  title: 'Update personal project order',
  description:
    "Move a project before/after another one in the caller's own arrangement of that space (see `GET /base/access/all?orderBy=personal`). The first move in a space freezes the order the caller currently sees; nobody else is affected.",
  request: {
    params: z.object({
      baseId: z.string(),
    }),
...
export const resetBasePersonalOrderRoute: RouteConfig = registerRoute({
  ...
  title: 'Reset personal project order',
  description:
    "Forget the caller's own arrangement of a space's projects; the personal list goes back to last-visit recency.",
  request: {
    params: z.object({
      spaceId: z.string(),
    }),
