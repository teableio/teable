...
describe('Realtime Ops (e2e)', () => {
  ...
  it.skip('pushes ops when deleting intermediate formula B (C and D become null)', async () => {
    const table = await createTable(baseId, { name: 'del-mid-B', records: [{ fields: {} }] });
    const tableId = table.id;
    const rec = (await getRecords(tableId)).records[0];

    const A = await createField(tableId, { type: FieldType.Number });
    await updateRecord(tableId, rec.id, {
      fieldKeyType: FieldKeyType.Id,
      record: { fields: { [A.id]: 2 } },
    });
    const B = await createField(tableId, {
      type: FieldType.Formula,
      options: { expression: `{${A.id}} + 1` },
    });
    const C = await createField(tableId, {
      type: FieldType.Formula,
      options: { expression: `{${B.id}} * 2` },
    });
    const D = await createField(tableId, {
      type: FieldType.Formula,
      options: { expression: `{${C.id}} + 3` },
    });
    ...
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('timeout waiting for C,D null')), 8000);
      const got = { c: false, d: false };
      const handler = (ops: any[]) => {
        const hitC = ops?.find((op) => Array.isArray(op.p) && op.p[1] === C.id);
        const hitD = ops?.find((op) => Array.isArray(op.p) && op.p[1] === D.id);
        try {
          if (hitC && Object.prototype.hasOwnProperty.call(hitC, 'oi')) {
            expect(hitC.oi).toBeNull();
            got.c = true;
          }
          if (hitD && Object.prototype.hasOwnProperty.call(hitD, 'oi')) {
            expect(hitD.oi).toBeNull();
            got.d = true;
          }
          if (got.c && got.d) {
            clearTimeout(timer);
      ...
      doc.on('op', handler as any);
      deleteField(tableId, B.id).catch((e) => {
        clearTimeout(timer);
        doc.removeListener('op', handler as any);
        reject(e);
      });
  ...
  it.skip('pushes ops when deleting intermediate formula C (D becomes null)', async () => {
    const table = await createTable(baseId, { name: 'del-mid-C', records: [{ fields: {} }] });
    const tableId = table.id;
    const rec = (await getRecords(tableId)).records[0];
    ...
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('timeout waiting for D null')), 8000);
      const handler = (ops: any[]) => {
        const hitD = ops?.find((op) => Array.isArray(op.p) && op.p[1] === D.id);
        if (hitD && Object.prototype.hasOwnProperty.call(hitD, 'oi')) {
          try {
      ...
      doc.on('op', handler);
      deleteField(tableId, C.id).catch((e) => {
        clearTimeout(timer);
        doc.off('op', handler);
        reject(e);
      });
