...
describe('Formula FROMNOW / TONOW (e2e)', () => {
  ...
  it('supports unit conversion and keeps TONOW past-positive semantics', async () => {
    ...
    try {
      ...
      expect(pastHour).toBeGreaterThanOrEqual(pastDay * 24);
      expect(pastHour).toBeLessThan((pastDay + 1) * 24);
      expect(pastSecond).toBeGreaterThanOrEqual(pastHour * 3600);
      expect(pastSecond).toBeLessThan((pastHour + 1) * 3600);
