...
describeByodbStorage('BYODB space storage placement (e2e)', () => {
  ...
  const assertDotTeaBaseImportRouting = async (targetSpaceId: string) => {
    ...
    try {
      ...
      await expect(countDbTableRows(dataDb, importedHost.dbTableName)).resolves.toBe(1);
      await expect(countDbTableRows(dataDb, importedForeign.dbTableName)).resolves.toBe(1);
