import { DateFormattingPreset, type IDateFieldOptions, TimeFormatting } from '@teable/core';
...
const isUnitAllowed = (
  unit: IDateSearchUnit,
  formatting: IDateFieldOptions['formatting']
): boolean => {
  const dateFormat = formatting.date ?? DateFormattingPreset.ISO;
  const hasTime = formatting.time != null && formatting.time !== TimeFormatting.None;
...
export const getDateSearchRange = (
  ...
  const formatting = dateFieldOptions.formatting;
