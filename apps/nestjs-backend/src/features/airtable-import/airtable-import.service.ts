...
export class AirtableImportService {
  ...
  async importBase(
    ...
    if (!ro.baseId) progress({ phase: 'creating_base', detail: ro.baseName });
    const base = ro.baseId
      ? await this.baseService.getBaseById(ro.baseId)
      : await this.baseService.createBase({
          spaceId: ro.spaceId,
