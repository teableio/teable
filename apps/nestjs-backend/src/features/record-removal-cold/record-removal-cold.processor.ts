...
// NEVER share record-history's queue: the two subsystems kill-switch, scale
// and roll back independently
export const RECORD_REMOVAL_COLD_QUEUE = 'record-removal-cold-queue';

const FLUSH_JOB_ID = 'record-removal-cold:flush';
const FLUSH_INTERVAL_MS = 24 * 60 * 60 * 1000;
const COMPACT_JOB_ID = 'record-removal-cold:compact';
// 04:40 UTC on the 3rd of each month: every closed month has fully flushed,
// and the slot is offset from record-history's compaction ('10 4 2 * *') so
// the two subsystems' month merges never contend for the same worker window
const COMPACT_CRON = '40 4 3 * *';
...
const CATCHUP_JOB_ID_PREFIX = 'record-removal-cold-flush-catchup';

// Daily incremental flush plus monthly compaction of the record_trash
...
export class RecordRemovalColdProcessor extends WorkerHost {
  ...
  async onApplicationBootstrap() {
    ...
    if (!config.flushSchedulerEnabled && !config.compactSchedulerEnabled) {
      ...
      if (typeof this.worker?.pause === 'function') {
        ...
        this.logger.log('record-removal cold worker paused (cold feature disabled here)');
      }
    ...
    if (typeof this.queue.upsertJobScheduler !== 'function') {
      this.logger.warn('record-removal cold schedulers unavailable without redis');
      return;
    }
    ...
    try {
      if (config.flushSchedulerEnabled) {
        ...
        // into a footgun (see the 2026-07-08 record-history stalls). If a
        // backlog must drain sooner than the next daily slot, run the EE
        // cold runner once (flush --max-rows=0 --max-bytes=0) — a
        // deliberate op, not boot magic.
        await this.queue.upsertJobScheduler(
          FLUSH_JOB_ID,
          { every: FLUSH_INTERVAL_MS },
          { name: FLUSH_JOB_ID }
        );
        this.logger.log(`record-removal cold flush scheduled (every ${FLUSH_INTERVAL_MS / 1000}s)`);
      }
      if (config.compactSchedulerEnabled) {
        ...
        this.logger.log(`record-removal cold compaction scheduled (cron ${COMPACT_CRON})`);
      }
    } catch (error) {
      this.logger.error('failed to register record-removal cold schedulers', error);
    }
  }
  ...
  async process(job: Job): Promise<IColdFlushRunResult | ICompactMonthResult[] | undefined> {
    ...
    if (!config.flushSchedulerEnabled && !config.compactSchedulerEnabled) {
      this.logger.warn(
        'skipping removal cold maintenance job: this process has no cold scheduler flags'
      );
      return undefined;
    }
    ...
    // bookmarks, so a space whose activity signal was ever missed — and any
    // rows that aged past their horizon while the space sat idle — is
    // stranded for at most a month instead of forever
    const result = await this.flusher.runFlush({
      mode: 'incremental',
      ignoreBookmarks: new Date().getUTCDate() === 1,
    });
    this.logger.log(
      `record-removal cold flush: tables=${result.tables.length} rows=${result.totalRows} ` +
        `parts=${result.totalParts} bytes=${result.totalCompressedBytes} in ${result.durationMs}ms` +
        (result.totalTruncatedRows ? ` truncated=${result.totalTruncatedRows}` : '') +
        (result.leftoverTables ? ` (deferred ${result.leftoverTables} unit(s))` : '') +
  ...
  onFailed(job: Job | undefined, error: Error) {
    this.logger.error(
      `record-removal cold job ${job?.name ?? 'unknown'} failed: ${error?.stack ?? error}`
    );
  }
