...
const createSnapshotItem = (fieldId: string) => ({
...
describe('V2FieldDeleteSnapshotSink', () => {
  it('prepares v2 delete snapshots, frozen view ops, and references on the execution context', async () => {
    ...
    const sink = new V2FieldDeleteSnapshotSink(tableMapper as never, db as never);
    const context = {
    ...
    const compatContext = (context as Record<string, unknown>)[
      V2_FIELD_DELETE_COMPAT_CONTEXT_KEY
    ] as {
      tableId: string;
      userId: string;
      remainingFieldIds: Set<string>;
      snapshots: ReadonlyArray<{ snapshot: { field: { id: string } } }>;
      referencesByFieldId: ReadonlyMap<string, ReadonlyArray<string>>;
      frozenFieldOps: Record<string, unknown[]>;
    };

    expect(compatContext.tableId).toBe('tblCompatTable0001');
    expect(compatContext.userId).toBe('usrCompatWriter00001');
    expect([...compatContext.remainingFieldIds]).toEqual([
      'fldCompatA00000001',
      'fldCompatB00000001',
    ]);
    expect(compatContext.snapshots.map((item) => item.snapshot.field.id)).toEqual([
      'fldCompatA00000001',
      'fldCompatB00000001',
    ]);
    expect(compatContext.referencesByFieldId.get('fldCompatA00000001')).toEqual([
      'fldDependent0000001',
    ]);
    expect(compatContext.frozenFieldOps.viwCompat000000001).toHaveLength(1);
  });
});
...
describe('V2FieldDeletedCompatProjection', () => {
  ...
  it('writes conditional lookup snapshots in the legacy field shape', async () => {
    ...
    const compatContext = {
      ...
      completed: undefined as boolean | undefined,
      remainingFieldIds: new Set(['fldCondLookup000001']),
      frozenFieldOps: {},
      snapshots: [
        {
          table: { kind: 'domainTable' },
    ...
    const result = await projection.handle(executionContext, {
      tableId: { toString: () => 'tblCompatTable0001' },
      fieldId: { toString: () => 'fldCondLookup000001' },
    } as never);
