...
const stableAdmissionDeferDelayMs = (baseId: string, taskId: string): number => {
  ...
  for (const char of `${baseId}:${taskId}`) {
    hash ^= char.charCodeAt(0); // NOSONAR typescript:S7758 -- the hash is defined over UTF-16 code units; switching to code points would change persisted/compared values
