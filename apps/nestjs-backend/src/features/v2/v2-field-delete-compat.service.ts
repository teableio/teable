...
import { FieldDeleted, ProjectionHandler, domainError, ok, v2CoreTokens } from '@teable/v2-core';
import type {
  DomainError,
...
const legacyPayloadFromSnapshots = (
  ...
  for (const { table, snapshot } of snapshots) {
    ...
    const tableDto = tableDtoResult.value;
...
export class V2FieldDeleteSnapshotSink implements IFieldDeleteSnapshotSink {
  ...
  async prepare(
    ...
    (
      ...
      remainingFieldIds: new Set(input.fieldIds),
...
export class V2FieldDeletedCompatProjection implements IEventHandler<FieldDeleted> {
  ...
  async handle(
    ...
    const legacyPayloadResult = legacyPayloadFromSnapshots(
      tableMapper,
      compatContext.snapshots,
      compatContext.referencesByFieldId
    );
...
export class V2FieldDeleteCompatService implements IV2ProjectionRegistrar {
  ...
  registerProjections(container: DependencyContainer): void {
    ...
    container.registerInstance(
      ...
      new V2FieldDeleteSnapshotSink(tableMapper, metaDb)
    );
    container.registerInstance(
