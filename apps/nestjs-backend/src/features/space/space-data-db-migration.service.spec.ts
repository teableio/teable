...
describe('SpaceDataDbMigrationService', () => {
  ...
  beforeEach(() => {
    ...
    prismaService.spaceDataDbMigrationJob.findUnique.mockReset();
    ...
    sourceClient.raw.mockReset().mockResolvedValue({ rows: [] });
  ...
  it('does not clean retry target artifacts when the previous job inventory differs', async () => {
    ...
    targetClient.raw.mockResolvedValueOnce({ rows: [{ schemaName: 'bsexxx' }] });
    const service = createService();
  ...
  it('rejects migrate-space when the source needs pg_trgm but the target lacks it', async () => {
  ...
  it('starts migrate-space when the target has the required pg_trgm operator class', async () => {
    sourceDataPrisma.$queryRawUnsafe.mockImplementation((query: string) => {
      if (query.includes('pg_get_indexdef')) {
        return Promise.resolve([
          {
  ...
  it('starts and switches a migration for cross-space linked spaces together', async () => {
    ...
    prismaService.tableMeta.findMany
      .mockResolvedValueOnce([
        {
          id: 'tblrelated',
      ...
      .mockResolvedValueOnce([
        { id: 'tblxxx', dbTableName: 'bsexxx.sheet1' },
        { id: 'tblrelated', dbTableName: 'bserelated.sheet1' },
      ]);
  ...
  it('starts a repair migration for default spaces linked to an existing target-bound space', async () => {
    ...
    prismaService.tableMeta.findMany
      ...
      .mockResolvedValueOnce([{ id: 'tblrelated', dbTableName: 'bserelated.sheet1' }])
      .mockResolvedValueOnce([{ id: 'tblrelated' }, { id: 'tbldeletedrelated' }])
      .mockResolvedValueOnce([
        { id: 'tblalready' },
        { id: 'tbldeletedalready' },
        { id: 'tblrelated' },
        { id: 'tbldeletedrelated' },
      ]);
    prismaService.space.findMany.mockResolvedValue([
      {
        id: 'spcalready',
  ...
  it('claims the oldest pending migration job for a worker', async () => {
    prismaService.spaceDataDbMigrationJob.findFirst.mockResolvedValue({ id: 'sdmjpending' });
  ...
  it('does not claim a migration job when another worker wins the pending update', async () => {
    prismaService.spaceDataDbMigrationJob.findFirst.mockResolvedValue({ id: 'sdmjpending' });
  ...
  it('truncates target shared tables when the target connection is unbound', async () => {
    ...
    expect(targetClient.raw).toHaveBeenCalledWith(
      `TRUNCATE TABLE "${internalSchema}"."record_history"`
    );
    expect(
      targetClient.raw.mock.calls.some(
  ...
  it('fails validation when the target data DB baseline schema version is not current', async () => {
    ...
    targetClient.raw.mockImplementation((sql: string) => {
      ...
      if (sql.includes('FROM pg_attribute a')) {
        return { rows: columnSignatureRows() };
      }
      if (sql.includes('FROM "bsexxx"."sheet1"')) {
        return { rows: [{ count: '3' }] };
      }
  ...
  it('fails before copy when the source inventory changed after freeze', async () => {
    ...
    prismaService.tableMeta.findMany.mockResolvedValue([
      { id: 'tblnew', dbTableName: 'bsexxx.sheet2' },
    ]);
    const service = createService();
