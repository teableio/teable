...
export class SpaceDataDbMigrationService {
  ...
  private deltaTriggerName(jobId: string) {
    return `${deltaTriggerNamePrefix}${jobId.replace(/[^a-zA-Z0-9_]/g, '_').slice(-32)}`;
  }
  ...
  private async closeSourceSnapshot(snapshot: ISourceSnapshotHandle | null) {
    ...
    try {
      await snapshot.client.raw('ROLLBACK');
    } finally {
  ...
  private targetSchemaForDeltaRow(row: ISourceDeltaRow, sourceSchema: string, targetSchema: string) {
  ...
  private async applyDeltaRowToTarget(input: {
    ...
    if (input.row.op === 'DELETE') {
      await input.targetClient.raw(
        `DELETE FROM ${qualified} WHERE ${quoteIdent(pkColumn)} = ?`,
        [pkValue]
      );
      return true;
    }
      ...
      .update({
        ...
        data: {
          copyStats: {
            ...(this.asRecord(job.copyStats) ?? {}),
            phase: stats.phase,
            progress: this.buildMigrationProgress(job, inventory, stats.phase),
            delta: stats,
          },
          lastError: null,
        },
  ...
  private async replayDeltaForJob(
    ...
    const batchSize =
      options.batchSize ??
      readPositiveIntEnv('BYODB_SPACE_DATA_DB_DELTA_REPLAY_BATCH_SIZE', defaultDeltaReplayBatchSize);
    const lagThreshold =
  ...
  async validateCopyForJob(
    ...
    try {
      ...
      const validationEndSeq = await this.tryGetSourceDeltaMaxSeq(sourceClient, sourceSchema, jobId);
