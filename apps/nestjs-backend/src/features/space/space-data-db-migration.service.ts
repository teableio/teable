...
export class SpaceDataDbMigrationService {
  ...
  async startMigrationForSpace(spaceId: string, createdBy: string, dataDb: IDataDbPreflightRo) {
    ...
    const internalSchema = this.resolveMigrationTargetInternalSchema(
      dataDb,
      relatedSpaces,
    ...
    await this.assertTargetHasNoSpaceConflicts(dataDb.url, internalSchema, inventory, spaceId);
    await this.assertTargetSupportsSourceDependencies(dataDb.url, inventory, spaceId);
  ...
  async claimNextPendingMigrationJob(workerId: string) {
    const pendingJob = (await this.migrationJobClient.spaceDataDbMigrationJob.findFirst({
      where: {
        targetMode: migrateSpaceTargetMode,
        state: 'pending',
      },
  ...
  async recoverStaleActiveMigrationJobs(
    ...
    const states = activeSpaceDataDbMigrationStates.filter((state) => state !== 'pending');
    ...
    for (const job of jobs) {
      const lastError = `Space data database migration job ${job.id} is stale; no worker progress since ${
        job.lastModifiedTime?.toISOString() ?? 'never'
      }`;
  ...
  private async cleanupRetryableTargetArtifacts(input: {
    ...
    const retryableJob = (await this.migrationJobClient.spaceDataDbMigrationJob.findFirst({
      where: {
      ...
      select: { id: true, spaceId: true, state: true, targetConnectionId: true, inventory: true },
  ...
  private buildProcessFailureStats(error: unknown) {
  ...
  private async assertTargetSupportsSourceDependencies(
    ...
    try {
      ...
      const errors = missingExtensions.map((dependency) => ({
        ...
        message: `Target database is missing ${dependency.schemaName}.${dependency.objectName}, required by source ${dependency.extensionName} indexes`,
        remediation:
          dependency.extensionName === 'pg_trgm'
            ? `Install pg_trgm in the target database before migration: CREATE EXTENSION IF NOT EXISTS pg_trgm WITH SCHEMA ${dependency.schemaName};`
  ...
  private async cleanupTargetSharedRows(
    ...
    for (const plan of plans) {
      ...
      if (options.truncate) {
        await client.raw(`TRUNCATE TABLE ${qualifiedTable}`);
        stats.push({
          table: plan.table,
          deletedRows: null,
          truncated: true,
