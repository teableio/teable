...
@Injectable()
// `@nestjs/passport` 12 checks the constructor arguments against `@types/passport-apple`, which
// models `store` with stricter callbacks than passport-oauth2 accepts at runtime. Widen the
// strategy type so the options keep their runtime shape.
export class AppleStrategy extends PassportStrategy(
  Strategy as Type<InstanceType<typeof Strategy>>,
  'apple',
  true
) {
  constructor(
