...
export class MobileAuthController {
  ...
  async exchange(
    ...
    await this.sessionService.recordSignin(req, 'mobile');
    return user;
  }
   ...
   * that minted it is alive. A browser context that arrives with the native session itself is
   * left on it; any other context gets a new session tied to that native session.
   */
  @Public()
  @Get('web-session')
  async webSession(
    ...
    if (req.sessionID !== parentSessionId) {
      // `req.login` regenerates the session, i.e. destroys the one this request arrived with.
      // A WebView that shares the app's cookie jar arrives with the native session itself, so
      // logging it in again would kill the session the app is still using (T7131): only that
      // parent session is left alone. Any other context — none, or a stale same-user session
      // from an earlier native sign-in — gets a fresh child session tied to this native one.
      await login(req, user);
      await this.sessionService.recordSignin(req, 'mobile_webview');
      await this.mobileAuthService.registerChildSession(parentSessionId, req.sessionID);
      await this.mobileAuthService.recordSignin(user.id);
    }
    const target =
