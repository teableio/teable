...
describe('MobileAuthService', () => {
  ...
  it('binds a single-use code to the user and challenge and returns the app redirect', async () => {
    ...
    // The exchange signs the app in: last sign-in time + the user.signin audit row.
    expect(ctx.users.refreshLastSignTime).toHaveBeenCalledWith('usr1');
    expect(ctx.users.refreshLastSignTime).toHaveBeenCalledTimes(1);
  ...
  it('burns the code when the verifier is wrong', async () => {
    ...
    expect(ctx.users.refreshLastSignTime).not.toHaveBeenCalled();
  ...
  it('rejects codes of deactivated or missing users', async () => {
    ...
    expect(deactivated.users.refreshLastSignTime).not.toHaveBeenCalled();
  });
