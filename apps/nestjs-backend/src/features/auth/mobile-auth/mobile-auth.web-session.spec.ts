...
function makeController() {
  ...
  return { controller, service, sessionService };
}
...
describe('MobileAuthController.webSession', () => {
  it('signs a browser context without a session in as a child of the native session', async () => {
    const { controller, service, sessionService } = makeController();
    ...
    expect(sessionService.recordSignin).toHaveBeenCalledWith(req, 'mobile_webview');
    expect(service.registerChildSession).toHaveBeenCalledWith(NATIVE_SID, CHILD_SID);
    // A new browser session is a sign-in: it writes user.signin like the other paths.
    expect(service.recordSignin).toHaveBeenCalledWith(user.id);
  ...
  it('leaves a browser context that already shares the native session on it', async () => {
    ...
    expect(sessionService.recordSignin).not.toHaveBeenCalled();
    expect(req.sessionID).toBe(NATIVE_SID);
    expect(service.registerChildSession).not.toHaveBeenCalled();
    // No new session, so no sign-in to record.
    expect(service.recordSignin).not.toHaveBeenCalled();
    expect(res.redirect).toHaveBeenCalledWith(query.redirect);
  });
