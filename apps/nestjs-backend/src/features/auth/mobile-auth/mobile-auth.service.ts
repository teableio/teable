...
export class MobileAuthService {
  ...
  async exchange(ro: IExchangeMobileAuthCodeRo): Promise<IUserMeVo> {
    ...
    await this.recordSignin(user.id);
    return user;
  }
  ...
  /**
   * A new app or WebView session is a sign-in like any other: refresh the last sign-in time and
   * write the `user.signin` audit row, the same way the password and SSO strategies do.
   */
  async recordSignin(userId: string): Promise<void> {
    await this.userService.refreshLastSignTime(userId);
  }
