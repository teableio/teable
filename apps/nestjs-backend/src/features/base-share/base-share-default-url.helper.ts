...
): { resourceType: BaseNodeResourceType; resourceId: string } | null => {
  const children = allNodes
    .filter((n) => n.parentId === parentNodeId)
    .sort((a, b) => a.order - b.order);

  for (const child of children) {
    if (child.resourceType !== BaseNodeResourceType.Folder) {
...
export const buildBaseShareDefaultUrl = async (
  ...
  const rows = await prisma.baseNode.findMany({
    where: { baseId },
    select: {
      id: true,
      parentId: true,
  ...
  const allNodes: IBaseNodeRow[] = rows.map((row) => ({
    ...row,
    resourceType: row.resourceType as BaseNodeResourceType,
  }));
  ...
  let targetNode: { resourceType: BaseNodeResourceType; resourceId: string } | null = null;
  ...
  if (nodeId === null) {
    ...
    if (sharedNode) {
      ...
      if (sharedNode.resourceType === BaseNodeResourceType.Folder) {
  ...
  const { resourceType, resourceId } = targetNode;
  if (resourceType === BaseNodeResourceType.Table) {
    return (
      (await buildTableUrl(prisma, baseId, resourceId)) ??
      buildBaseNodeUrl(baseId, resourceType, resourceId)
    );
  }
  return buildBaseNodeUrl(baseId, resourceType, resourceId);
};
