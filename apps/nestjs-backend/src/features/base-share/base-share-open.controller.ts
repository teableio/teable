...
export class BaseShareOpenController {
  ...
  private async runShareBaseCopy(
    ...
    // Audit rows emitted by atomic events inside baseDuplicateService.duplicateBase. A copy into a
    // new base also records that base (no controller BASE_CREATE event fires on this route) — with
    // where it came from, since copying a shared base takes its data out of the sharer's space.
    if (!targetBaseId) {
