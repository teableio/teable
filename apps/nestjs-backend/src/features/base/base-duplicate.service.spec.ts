...
describe('BaseDuplicateService duplicateBaseV2', () => {
  ...
  type IDuplicateServiceInternals = {
    ...
    getDisconnectedLinkFieldTableMap: (
      ...args: unknown[]
    ) => Promise<
      Record<string, { dbFieldName: string; selfKeyName: string; isMultipleCellValue: boolean }[]>
    >;
  ...
  it('should create the v2 execution context from the space data container', async () => {
    ...
    const baseImportService = {
      ...
      restoreBaseExtrasV2: vi.fn().mockResolvedValue(undefined),
    };
