...
export class BaseDuplicateService {
  ...
  async duplicateBaseV2(
    ...
    try {
      ...
      await this.baseImportService.restoreBaseExtrasV2(
        db,
        base.id,
        structure,
      ...
      return { base, tableIdMap, fieldIdMap, viewIdMap, recordsLength, structure };
    } catch (error) {
      if (base?.id) {
        await prisma.base
          .update({
  ...
  private async getDisconnectedLinkFieldTableMap(
    ...
    const toLinkInfo = ({
      ...
      options,
      isMultipleCellValue,
    }: (typeof disconnectedLinkFields)[number]): ILinkFieldTableInfo => ({
      dbFieldName,
    ...
    Object.entries(groupBy(disconnectedLinkFields, 'tableId')).forEach(([tableId, fields]) => {
      const info = fields.map(toLinkInfo);
  ...
  private async getCrossBaseLinkFieldTableMap(
    ...
    const toLinkInfo = ({
      ...
      options,
      isMultipleCellValue,
    }: (typeof crossBaseLinkFields)[number]): ILinkFieldTableInfo => ({
      dbFieldName,
      selfKeyName: (options as ILinkFieldOptions).selfKeyName,
      isMultipleCellValue: !!isMultipleCellValue,
    });
