...
import { nextReadBatchLimit } from './record-history-flusher.service';
...
describe('record-history cold storage', () => {
  ...
  describe('external row sorter', () => {
    ...
    it('read batch limit re-targets the byte goal from observed row weight', () => {
      ...
      // ~1MB rows: floors at the minimum instead of materializing gigabytes
      expect(nextReadBatchLimit(500 * 1024 * 1024, 500, 5000)).toBe(100);
      // 4KB rows: lands at 8MB target / 4KB = 2048
      expect(nextReadBatchLimit(500 * 4096, 500, 5000)).toBe(2048);
      // a cap set BELOW the fat-row floor stays the hard ceiling
      expect(nextReadBatchLimit(500 * 1024 * 1024, 500, 50)).toBe(50);
    });
  });
