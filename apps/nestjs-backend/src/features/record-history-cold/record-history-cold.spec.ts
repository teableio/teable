  ...
  describe('cold maintenance processor', () => {
    class FakeColdQueue {
      ...
      async add(name: string, data: unknown, opts?: { jobId?: string }) {
        ...
        // (completed and failed ones included) makes the add a no-op
        if (opts?.jobId && this.jobs.some((job) => job.id === opts.jobId)) {
          return;
        }
        this.jobs.push({ id: opts?.jobId, name, data, state: 'delayed', opts });
      }
    ...
    it('queues a boot flush on upgrade deploys but not on first install', async () => {
      const fresh = new FakeColdQueue();
      await makeProcessor(fresh).onApplicationBootstrap();
      // first install: the just-created scheduler fires its first run itself
      expect(fresh.jobs).toHaveLength(0);

      const upgraded = new FakeColdQueue();
      upgraded.schedulers.push({ key: 'record-history-cold:flush' });
      await makeProcessor(upgraded).onApplicationBootstrap();
      expect(upgraded.jobs.map((job) => job.id)).toEqual(['record-history-cold-flush-boot']);
      // the completed job doubles as the fleet-wide dedupe marker across a
      // slow rolling deploy, so it must outlive its own completion
      expect(upgraded.jobs[0].opts).toMatchObject({ removeOnComplete: { age: 3600 } });
    });

    it('dedupes the boot kick against a marker left by an earlier pod', async () => {
      const queue = new FakeColdQueue();
      queue.schedulers.push({ key: 'record-history-cold:flush' });
      // first pod's kick already ran to completion but its job was retained
      queue.jobs.push({
        id: 'record-history-cold-flush-boot',
        name: 'record-history-cold:flush',
        data: {},
        state: 'completed',
      });
    ...
    it('drops a stale FAILED kick marker so a redeploy retries', async () => {
      const queue = new FakeColdQueue();
      queue.schedulers.push({ key: 'record-history-cold:flush' });
      // age-based removeOnFail prunes lazily, so on an idle queue this
      // marker would otherwise dedupe every future deploy's kick
      queue.jobs.push({
        id: 'record-history-cold-flush-boot',
        name: 'record-history-cold:flush',
        data: {},
      ...
      await makeProcessor(queue).onApplicationBootstrap();
      expect(queue.jobs.map((job) => job.state)).toEqual(['delayed']);
      expect(queue.jobs[0].id).toBe('record-history-cold-flush-boot');
    });

    it('skips the boot flush while a flush is already running or chained', async () => {
      const queue = new FakeColdQueue();
      queue.schedulers.push({ key: 'record-history-cold:flush' });
      queue.jobs.push({
        id: 'repeat:whatever:123',
        name: 'record-history-cold:flush',
        data: {},
        state: 'active',
      });
      ...
      const chained = new FakeColdQueue();
      chained.schedulers.push({ key: 'record-history-cold:flush' });
      chained.jobs.push({
      ...
      await makeProcessor(chained).onApplicationBootstrap();
      expect(chained.jobs).toHaveLength(1);
    });

    it('ignores the scheduler iteration itself when deciding to kick', async () => {
      const queue = new FakeColdQueue();
      queue.schedulers.push({ key: 'record-history-cold:flush' });
      // the scheduler's own next run sits delayed up to a day away — exactly
      // the job the boot kick exists to shortcut, so it must not block it
      queue.jobs.push({
        id: 'repeat:record-history-cold:flush:1783588736669',
        name: 'record-history-cold:flush',
        data: {},
        state: 'delayed',
      });
      await makeProcessor(queue).onApplicationBootstrap();
      expect(queue.jobs.map((job) => job.id)).toContain('record-history-cold-flush-boot');
    });
  });
});
