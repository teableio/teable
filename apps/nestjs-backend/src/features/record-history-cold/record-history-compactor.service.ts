...
export class RecordHistoryCompactorService {
  ...
  async compactMonth(
    ...
    const rows = await this.mergeInputs(inputs, writer);
  ...
  private async mergeInputs(inputs: IParsedPartKey[], writer: PartWriter): Promise<number> {
    const sorter = new ExternalRowSorter();
    try {
      for (const input of inputs) {
        for await (const item of this.coldStorage.iterateRows(input.key)) {
          if (item.row) await sorter.add(item.row);
        }
      }
      let rows = 0;
