...
const BOOT_FLUSH_JOB_ID = 'record-history-cold-flush-boot';
const BOOT_FLUSH_DELAY_MS = 60 * 1000;
/** keep the finished boot job as a dedupe marker across a slow rolling deploy */
const BOOT_FLUSH_DEDUPE_AGE_S = 60 * 60;
...
export class RecordHistoryColdProcessor extends WorkerHost {
  ...
  async onApplicationBootstrap() {
    ...
    try {
      if (config.flushSchedulerEnabled) {
        const hadFlushScheduler = await this.hasFlushScheduler();
        ...
        // the `every` scheduler fires immediately only when first CREATED; on
        // an upgrade deploy the next slot can be up to a day away, which would
        // strand a backlog behind the very fix that ships in that deploy
        if (hadFlushScheduler) {
          await this.kickBootFlush();
        }
      }
  ...
  /** whether the flush scheduler already existed before this boot (upgrade vs first install) */
  private async hasFlushScheduler(): Promise<boolean> {
    const queue = this.queue as Queue & {
      getJobSchedulers?: (
        start?: number,
        end?: number
      ) => Promise<({ key?: string; id?: string } | undefined)[]>;
    };
    // conservative default: treating an unknown state as "upgrade" only risks
    // one extra budgeted run, while "first install" would skip the kick that
    // upgrade deploys rely on to resume a stalled backlog
    ...
    try {
      ...
      return schedulers.some(
        (scheduler) => scheduler?.key === FLUSH_JOB_ID || scheduler?.id === FLUSH_JOB_ID
      );
    } catch {
      return true;
    }
  ...
  /**
   * one budgeted flush shortly after an upgrade boot. The fixed job id dedups
   * the kick across a rolling fleet; a flush that is already running or an
   * already-chained catch-up makes the kick redundant, so those skip it. The
   * scheduler's own far-future iteration (a delayed job with a generated
   * repeat id) must NOT block it — that job is exactly the one being too far
   * away.
   */
  private async kickBootFlush(): Promise<void> {
    try {
      const queue = this.queue as Queue & {
        getJobs?: (types: string[]) => Promise<({ id?: string; name?: string } | undefined)[]>;
        remove?: (jobId: string) => Promise<number>;
      };
      if (typeof queue.getJobs === 'function') {
        const running = await queue.getJobs(['active', 'waiting']);
        if (running.some((other) => other?.name === FLUSH_JOB_ID)) {
          return;
        }
        const delayed = await queue.getJobs(['delayed']);
        const pendingKick = delayed.some(
          (other) => other?.id === BOOT_FLUSH_JOB_ID || other?.id?.startsWith(CATCHUP_JOB_ID_PREFIX)
        );
        if (pendingKick) {
          return;
        }
        // BullMQ prunes age-based removeOnFail lazily (only when a later job
        // finalizes), so on an otherwise idle queue a FAILED kick would keep
        // deduping every future add() until the next daily run. A redeploy is
        // exactly the moment a failed kick deserves another go — drop the
        // stale marker before re-adding. (A COMPLETED marker stays: that
        // dedupe is the rolling-deploy protection above.)
        if (typeof queue.remove === 'function') {
          const failed = await queue.getJobs(['failed']);
          if (failed.some((other) => other?.id === BOOT_FLUSH_JOB_ID)) {
            await queue.remove(BOOT_FLUSH_JOB_ID);
          }
        }
      }
      await this.queue.add(
        ...
        {},
        {
          delay: BOOT_FLUSH_DELAY_MS,
          jobId: BOOT_FLUSH_JOB_ID,
          // the finished job IS the fleet-wide dedupe marker: a rolling
          // deploy can outlive delay + one short run, and deleting the job at
          // completion would hand every late-booting pod a fresh kick.
          // (Catch-up hops below rightly delete immediately — their ids must
          // free up for the next day's chain.) Failed markers are dropped
          // explicitly on the next deploy above; the age here is hygiene.
          removeOnComplete: { age: BOOT_FLUSH_DEDUPE_AGE_S },
          removeOnFail: { age: BOOT_FLUSH_DEDUPE_AGE_S },
        }
      );
      this.logger.log('record-history cold boot flush queued');
    } catch (error) {
      this.logger.warn(`failed to queue boot flush: ${error}`);
    }
  }
