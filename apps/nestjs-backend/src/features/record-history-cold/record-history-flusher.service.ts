...
const READ_BATCH_PROBE_ROWS = 500;
const READ_BATCH_MIN_ROWS = 100;
  ...
  async flushTable(
    ...
    const feederFor = async (createdTime: string): Promise<BucketMergeFeeder> => {
      ...
      if (!feeder) {
        ...
        feeder = new BucketMergeFeeder(writer, existing, this.coldStorage, sortBudget);
    ...
    try {
      while (!streamNothing) {
        ...
        for (const row of batch) {
          ...
          batchBytes += approxColdRowBytes(coldRow);
