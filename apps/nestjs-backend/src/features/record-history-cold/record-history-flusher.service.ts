...
export class RecordHistoryFlusherService {
  ...
  private async readBatch(
    ...
    const params: unknown[] = [tableId, cutoff];
    ...
    if (ranges && ranges.length > 0) {
      const parts = ranges.map(
        (range) => `("created_time" >= ${push(range.lo)} AND "created_time" < ${push(range.hi)})`
      );
      rangeClause = ` AND (${parts.join(' OR ')})`;
    }
    let afterClause = '';
    if (after) {
      afterClause = ` AND ("record_id", "created_time", "id") > (${push(after.recordId)}, ${push(after.createdTime)}, ${push(after.id)})`;
    }
    // truncate over-cap values IN SQL so a multi-MB before/after never crosses
    // the wire into JS (materializing 64 x 15MB probe rows would OOM before any
    // in-process cap could run). char_length is computed once per value in the
    // inner scan; the marker mirrors coldTruncatedMarker(). cap<=0 disables.
    const cap = push(maxValueUnits);
    ...
    const rows = await this.databaseRouter.queryDataPrismaForTable(
      tableId,
      `SELECT "id", "recordId", "fieldId", "createdTime", "createdBy",
         CASE WHEN ${cap} > 0 AND "bl" > ${cap} THEN ${marker('"bl"')} ELSE "before" END AS "before",
         CASE WHEN ${cap} > 0 AND "al" > ${cap} THEN ${marker('"al"')} ELSE "after" END AS "after",
         (${cap} > 0 AND "bl" > ${cap}) AS "beforeTruncated",
         (${cap} > 0 AND "al" > ${cap}) AS "afterTruncated"
       FROM (
         ...
         FROM ${qualified}
         WHERE "table_id" = $1 AND "created_time" < $2${rangeClause}${afterClause}
         ORDER BY "record_id", "created_time", "id" LIMIT ${limit}
       ) "sub"`,
      ...params
    );
    return rows as {
