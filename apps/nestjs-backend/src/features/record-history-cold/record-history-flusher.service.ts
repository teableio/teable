...
export class RecordHistoryFlusherService {
  ...
  private async flushGroup(
    ...
    while (index < group.tableIds.length) {
      ...
      const sliceResults = await mapWithConcurrency(slice, run.concurrency, (tableId) =>
        this.flushTable(tableId, run.cutoff, run.mode, run.deleteEnabled, run.config).catch(
          (error): ITableFlushResult => {
            this.logger.error(
  ...
  async flushTable(
    ...
    const feederFor = async (createdTime: string): Promise<BucketMergeFeeder> => {
      ...
      if (!feeder) {
        ...
        feeder = new BucketMergeFeeder(writer, existing, this.coldStorage);
    ...
    while (!streamNothing) {
      ...
      for (const row of batch) {
        ...
        await (await feederFor(coldRow.createdTime)).push(coldRow);
        flushedRows += 1;
      }
      const last = batch[batch.length - 1];
      lastKey = { recordId: last.recordId, createdTime: last.createdTime, id: last.id };
      if (batch.length < config.readBatchSize) break;
    }
   ...
   * and survives for the next run — the range predicate alone would remove
   * it without it ever having been uploaded. Batched by ctid inside the
   * transaction to keep per-statement work bounded.
   */
        ...
        for (;;) {
          const affected = await tx.$executeRawUnsafe(
            `DELETE FROM ${qualified} WHERE ctid IN (
               SELECT ctid FROM ${qualified} WHERE "table_id" = $1 AND "created_time" < $2 LIMIT 10000
             )`,
            tableId,
            cutoff
          );
