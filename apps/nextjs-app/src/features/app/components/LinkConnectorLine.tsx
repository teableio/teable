...
export const LinkConnectorLine = () => {
  ...
  if (mode === 'top') {
    ...
    const tension = Math.max(dy, dx) * 0.45;
    const c1x = source.x;
    const c1y = source.y - tension;
    const c2x = target.x - tension;
    const c2y = target.y;
  ...
  return (
    <svg className="pointer-events-none fixed inset-0 z-[51]" width="100%" height="100%">
      <path
        d={path}
        fill="none"
