...
export const PoemGrid: FC<{ poems: SearchPoems; children?: never }> = (props) => {
  const { poems } = props;
  ...
  return (
    <div className="flex flex-wrap">
      {poems.map((poem) => {
        ...
        const defaultImg = `/_next/image?url=${encodeURIComponent(randomImg)}&w=640&q=85`;
        images = images.length < 1 ? waterImages : ArrayUtils.removeItem(images, randomImg);

        const unsplashImg = `https://source.unsplash.com/random/640x480?${(poem.keywords ?? [])
          .map((keyword) => encodeURIComponent(keyword))
          .join(',')}`;

        return (
          <div key={`${poem.id}`} className="m-2 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6">
            <PoemCard
              title={poem.title}
