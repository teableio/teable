...
export const TaskItem = ({ task, onCancel, onRemove, onRetry }: ITaskItemProps) => {
  ...
  return (
    <div
      className={cn(
        'group relative flex items-center gap-3 px-3 py-2.5 hover:bg-accent w-full overflow-hidden'
      )}
    >
      ...
      <div className="absolute right-0 top-0 z-10 hidden h-full items-center group-focus-within:flex group-hover:flex">
        <div className="h-full w-8 bg-gradient-to-r from-accent/0 to-accent" />
        <div className="flex h-full items-center gap-1 bg-accent py-1 pr-2">
          {isError ? (
