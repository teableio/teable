...
import {
  SettingTab as SpaceInnerSetting,
...
import { SettingTab as PersonalSettingTab } from '../../components/setting/useSettingStore';
...
export const SpaceInnerPage: React.FC = () => {
  ...
  const [settingDefaultTab, setSettingDefaultTab] = useState<
    SpaceInnerSetting | PersonalSettingTab
  >(SpaceInnerSetting.Plan);
  ...
  useEffect(() => {
    ...
    if (subscribeLevel && isCloud && space?.role === Role.Owner) {
      setSettingDefaultTab(
        host === 'self-hosted' ? PersonalSettingTab.LicensePlan : SpaceInnerSetting.Plan
      );
      setSettingModalOpen(true);
    }
