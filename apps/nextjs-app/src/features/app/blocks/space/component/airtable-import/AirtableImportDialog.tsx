...
import { Airtable, Check, Search } from '@teable/icons';
import {
...
import { openConnectIntegration } from '@/features/app/components/user-integration/utils';
...
const ViewConfigImportOption = (props: {
  ...
  return (
    <>
      ...
      {enabled && (
        <div className="ml-6 space-y-1.5 rounded-md border bg-muted/30 p-3">
          <p className="text-xs text-muted-foreground">
            {t('space:airtableImport.viewConfig.help')}
          </p>
          <Input
            value={shareLink}
            placeholder="https://airtable.com/appXXXXXXXX/shrXXXXXXXX"
...
export const AirtableImportDialog = (props: IAirtableImportDialogProps) => {
  ...
  const {
    ...
    queryKey: [...ReactQueryKeys.getUserIntegrations(), 'airtable-import', isWaitingOAuth],
    enabled: open,
    retry: false,
    refetchInterval: isWaitingOAuth ? 2000 : false,
    queryFn: async () =>
      (
  ...
  // Drive the detect step: connected integration -> straight to the base
  // picker (which loads the base list itself), none -> connect step. Also
  // picks up the integration created in the OAuth popup.
  React.useEffect(() => {
    ...
    if (!open || isDetectingFetch) return;
    if (detectedIntegration && (step === 'detect' || isWaitingOAuth)) {
      setIntegration(detectedIntegration);
      setIsWaitingOAuth(false);
      setStep('pick');
      return;
    }
    if (!detectedIntegration && step === 'detect') {
      setStep('connect');
    }
  }, [open, isDetectingFetch, detectedIntegration, step, isWaitingOAuth]);
  ...
  const handleConnectOAuth = () => {
    openConnectIntegration(UserIntegrationProvider.Airtable, { name: 'Airtable' });
        ...
        {step === 'connect' &&
          (integrationUnavailable ? (
            ...
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <Button onClick={handleConnectOAuth} disabled={isWaitingOAuth}>
                {isWaitingOAuth && <Spin className="mr-1 size-4" />}
                {isWaitingOAuth
        ...
        {step === 'pick' && (
          <div className="space-y-4">
            <div>
              ...
              {isLoadingBases ? (
                <div className="mt-2 flex h-72 items-center justify-center gap-2 rounded-lg border text-sm text-muted-foreground">
                  <Spin className="size-4" />
                  {t('space:airtableImport.loadingBases')}
                </div>
              ) : bases.length === 0 ? (
