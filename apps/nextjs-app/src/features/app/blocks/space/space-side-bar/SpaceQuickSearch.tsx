...
import { spaceSearch, ResourceType } from '@teable/openapi';
...
export const SpaceQuickSearch: FC<Props> = ({ spaceId }) => {
  ...
  const groupedResults = useMemo(() => groupBy(allResults, 'type'), [allResults]);

  const getHeading = (type: string) => {
    switch (type) {
      case ResourceType.Base:
        return t('common:noun.base');
  ...
  return (
    <>
      ...
      >
        ...
        />
        <CommandList>
          {debouncedSearch.length > 0 && !isLoading && Object.keys(groupedResults).length === 0 && (
            <CommandEmpty>{t('common:noResult')}</CommandEmpty>
          )}
          {Object.entries(groupedResults).map(([type, items]) => {
            const IconComponent = SearchTypeIconMap[type as keyof typeof SearchTypeIconMap];
            return (
              <CommandGroup heading={getHeading(type)} key={type}>
                {items.map((item) => (
                  <CommandItem
