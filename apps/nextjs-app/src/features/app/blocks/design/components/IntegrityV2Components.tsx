...
const RuleRepairAction = ({
  ...
  const { t } = useTranslation(['table']);
  const hasRepairTarget = Boolean(result.tableId && result.fieldId && result.ruleId);
  const canShowPreview = Boolean(onPreviewRepairRule && hasRepairTarget);
  const canRepair = Boolean(result.repair?.available && hasRepairTarget && onRepairRule);
  const reason = getLocalizedRepairReason(t as Translate, result);
  const description = getLocalizedRepairDescription(t as Translate, result);
  const tooltipReason = getRuleRepairTooltipText(t as Translate, result, reason, description);
    ...
    <div className="ml-auto flex items-center gap-2 opacity-0 transition-opacity duration-150 group-focus-within:opacity-100 group-hover:opacity-100">
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="inline-flex">
              <Button
                ...
                variant="outline"
                className="h-7 px-2 text-xs"
                disabled={!canShowPreview || isRunning || isPreviewLoading}
          ...
          <TooltipContent className="max-w-xs whitespace-normal">
            <div>{tooltipReason}</div>
            {reason && description ? <div className="mt-1 opacity-80">{description}</div> : null}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
