...
import type { ComputeActivityFieldClient, IComputeActivityState } from '@teable/sdk/hooks';
...
const FORMULA_COMPILE_CODES: Record<string, true> = {
...
const FieldActivityErrorText = ({
  ...
  if (!lastError) return t('computeActivity.calculationFailed');
  if (lastError.code === COMPUTED_CELL_VALUE_MAX_BYTES_CODE) {
    const attempted = lastError.context?.attempted;
    const max = lastError.context?.max;
    return t('computeActivity.cellValueTooLarge', {
      attempted: typeof attempted === 'number' ? attempted : '?',
      max: typeof max === 'number' ? max : '?',
    });
  }
  if (Object.hasOwn(FORMULA_COMPILE_CODES, lastError.code ?? ''))
    return t('computeActivity.formulaLimit');
  if (lastError.code === COMPUTED_RESOURCE_LIMIT_CODE) return t('computeActivity.resourceLimit');
  if (lastError.code === STAGE_DEPTH_EXHAUSTED_CODE)
    return t('computeActivity.stageDepthExhausted');
...
const getActivityStatusKey = (state: {
  ...
  if (state.hasFailedFields) return undefined;
  if (state.delayed) return 'computeActivity.delayed';
  return undefined;
};

// Observation loss is not evidence of a calculation failure. Show it only
// inside the details of known activity, never as a standalone warning.
const ActivityStatusMessage = ({
  observationState,
  isFetching,
  refetch,
}: Pick<IComputeActivityState, 'observationState' | 'isFetching' | 'refetch'>) => {
  const { t } = useTranslation('table');
  const descriptionId = useId();
  if (observationState !== 'syncing' && observationState !== 'unavailable') return null;
  const message =
    observationState === 'syncing'
      ? t('computeActivity.statusSyncing')
      : t('computeActivity.statusUnavailable');
  const description =
    observationState === 'syncing'
      ? t('computeActivity.statusSyncingDescription')
      : t('computeActivity.statusUnavailableDescription');

  return (
    <div className="space-y-2 text-xs text-muted-foreground">
      <p role="status" aria-describedby={descriptionId}>
        {message}
      </p>
      <p id={descriptionId}>{description}</p>
      <Button variant="outline" size="sm" disabled={isFetching} onClick={() => void refetch()}>
        {t('computeActivity.refreshStatus')}
      </Button>
    </div>
  );
};
...
export const ComputeActivityPanel = () => {
  const { t } = useTranslation('table');
  const { fieldMetaById, diagnostics, observationState, isFetching, refetch } =
    useComputeActivity();
  ...
  const statusKey = getActivityStatusKey({
    ...
    unknownImpact,
    pendingIssue,
    hasFailedFields: failed.length > 0,
    delayed,
  });
