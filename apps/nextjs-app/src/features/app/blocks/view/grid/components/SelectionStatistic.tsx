...
import { HelpCircle, Loader2 } from '@teable/icons';
import {
  getSelectionAggregation,
  SELECTION_AGGREGATION_MAX_TAKE,
...
const extractCellRange = (selection: CombinedSelection | undefined): IRange | null => {
...
export const computeSelectionStatistic = (
  ...
  const range = extractCellRange(selection);
...
const useBackendSelectionStats = (params: {
  ...
  const enabled =
    !!debounced &&
    !!tableId &&
    debounced.fieldIds.length > 0 &&
    take > 0 &&
    take <= SELECTION_AGGREGATION_MAX_TAKE;
...
const SelectionStatisticInner = (props: ISelectionStatisticProps) => {
  const { recordMap, columns, collapsedGroupIds } = props;
  ...
  const range = useMemo(() => extractCellRange(selection), [selection]);
  ...
  const computed = useMemo<IComputed | null>(() => {
    ...
    return computeSelectionStatistic(selection, recordMap, columns, fields);
  }, [range, needsBackend, backendData, selection, recordMap, columns, fields, aggregableFieldIds]);
    ...
    >
      ...
      <span>
        ...
        <span className="font-medium">
          {isLoading ? <LoadingValue /> : computed ? trimDecimals(computed.average) : '—'}
        </span>
      </span>
      <span className="h-3 w-px bg-border" />
      <span>
        ...
        <span className="font-medium">
          {isLoading ? <LoadingValue /> : computed ? computed.count : 0}
        </span>
      </span>
      <span className="h-3 w-px bg-border" />
      <span>
        ...
        <span className="font-medium">
          {isLoading ? (
            ...
            formatSum(computed.sum, computed.representativeField)
          ) : (
            '—'
          )}
        </span>
