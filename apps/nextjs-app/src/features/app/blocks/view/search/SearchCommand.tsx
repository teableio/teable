  ...
  return (
    <Command filter={commandFilter}>
      ...
      <CommandList className="my-2 max-h-64">
        {value !== 'all_fields' && <CommandEmpty>{t('listEmptyTips')}</CommandEmpty>}
        ...
        {value !== 'all_fields' &&
          fields.map((field) => {
            ...
            return (
              <CommandItem className="flex flex-1 p-0" key={id} value={id}>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    {selectedFields.includes(id) && selectedFields.length === 1 ? (
                      <TooltipContent>{t('atLeastOne', { noun: t('noun.field') })}</TooltipContent>
                    ) : null}
                  </Tooltip>
                </TooltipProvider>
              </CommandItem>
