...
export const CommunityPage = ({ buildBaseEnabled = true }: { buildBaseEnabled?: boolean }) => {
  const { t } = useTranslation(tableConfig.i18nNamespaces);
  const { baseUsage, loading, isFetched } = useBaseUsageWithLoading();
  const { level } = baseUsage || {};
  // free user or community user
  const displayDefaultBaseWelcome =
    level === BillingProductLevel.Free || level === undefined || !buildBaseEnabled;
