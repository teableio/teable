...
export const SignForm: FC<ISignForm> = (props) => {
  ...
  const { mutate: sendSigninCodeMutation, isPending: sendSigninCodeLoading } = useMutation({
    mutationFn: (email: string) => sendSigninVerificationCode(email),
    onSuccess: () => {
    ...
    >
      ...
      <form className="relative" onSubmit={onSubmit} onChange={() => setError(undefined)}>
        <div className="grid gap-4">
            ...
            <div className="grid gap-3">
              ...
              <SendVerificationButton
                ...
                onClick={(e) => {
                  ...
                  sendSigninCodeMutation(res.data.email);
                }}
              />
            </div>
          )}
          ...
          {/* Turnstile Widget: the code flow exists for browsers that cannot pass it */}
          {turnstileSiteKey && !isCodeSignin && (
