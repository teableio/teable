...
export class TableService {
  ...
  async getSSRSnapshot(tableId: string, viewId?: string) {
    ...
    const views = await viewService.getViews(tableId);
  ...
  private async createTable(prisma: Prisma.TransactionClient, snapshot: ITableSnapshot) {
    ...
    await fieldService.multipleCreateFieldsTransaction(
      prisma,
      tableId,
    ...
    await viewService.createViewTransaction(prisma, tableId, DEFAULT_VIEW);

    // 4. create records for table
    await recordService.multipleCreateRecordTransaction(prisma, tableId, DEFAULT_RECORDS);
