...
export class RecordService {
  ...
  private async getUserFields(
    ...
    if (userFields.length !== userFieldIds.length) {
      throw new Error('some fields not found');
    }
  ...
  async getRecords(tableId: string, query: RecordsRo): Promise<RecordsVo> {
    ...
    if (!viewId) {
      const defaultView = await prismaClient.view.findFirstOrThrow({
        where: { tableId },
        select: { id: true },
      });
    ...
    const ids = await this.getRecordDocIdsByQuery(prismaClient, tableId, {
    ...
    const total = await this.getRowCount(prismaClient, tableId, viewId);
