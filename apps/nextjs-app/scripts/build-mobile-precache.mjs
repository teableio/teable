 ...
 *
 * Inputs, all under `<app-dir>/.next`: `build-manifest.json`, `BUILD_ID`,
 * `react-loadable-manifest.json` (the `next/dynamic` chunks) and the asset
 * prefix the build used (`required-server-files.json`), so precache URLs point
 * at the CDN when one is configured.
 *
 * The emitted worker:
 *  - precaches every JS/CSS file of the routes the shell opens
 *    (MOBILE_PRECACHE_ROUTES) plus the shared `_app` chunks, polyfills, the
 *    `_buildManifest` / `_ssgManifest`, and the `next/dynamic` chunks those
 *    pages load after hydration (MOBILE_PRECACHE_DYNAMIC_MODULES — the grid
 *    itself is one, `ssr: false`, and nothing paints before it arrives);
...
/**
 * `next/dynamic` imports whose chunks belong in the shell too, matched against the
 * module specifier at the end of a react-loadable-manifest key
 * (`<importer> -> <specifier>`). The Table block is what every table page waits for.
 */
export const MOBILE_PRECACHE_DYNAMIC_MODULES = ['@/features/app/blocks/table/Table'];
...
/**
 * @typedef {Record<string, { id?: number | string; files?: string[] }>} ILoadableManifest
 */

/**
 * react-loadable manifest → the `.next/`-relative chunk files of the wanted
 * `next/dynamic` imports (any importer). Modules no entry matches are reported.
 *
 * @param {ILoadableManifest | undefined} manifest
 * @param {string[]} [modules]
 * @returns {{ files: string[]; missingModules: string[] }}
 */
export function collectDynamicChunkFiles(manifest, modules = MOBILE_PRECACHE_DYNAMIC_MODULES) {
  /** @type {Set<string>} */
  const files = new Set();
  /** @type {string[]} */
  const missingModules = [];
  for (const wanted of modules) {
    let found = false;
    for (const [key, entry] of Object.entries(manifest ?? {})) {
      const specifier = key.includes(' -> ') ? key.slice(key.lastIndexOf(' -> ') + 4) : key;
      if (specifier !== wanted) continue;
      found = true;
      for (const file of entry?.files ?? []) {
        if (ASSET_FILE_RE.test(file)) files.add(file);
      }
    }
    if (!found) missingModules.push(wanted);
  }
  return { files: [...files], missingModules };
}
...
export function buildMobileServiceWorker({ appDir = process.cwd(), outFile } = {}) {
  ...
  const loadablePath = join(nextDir, 'react-loadable-manifest.json');
  const loadable = existsSync(loadablePath)
    ? JSON.parse(readFileSync(loadablePath, 'utf8'))
    : undefined;
  const dynamic = collectDynamicChunkFiles(loadable);
  const allFiles = [...new Set([...files, ...dynamic.files])];
  const precacheUrls = toPrecacheUrls(allFiles, assetPrefix);
  ...
  return {
    ...
    precacheUrls,
    missingRoutes,
    missingModules: dynamic.missingModules,
  };
}
...
if (isDirectRun) {
  ...
  try {
    ...
    if (result.missingModules.length > 0) {
      console.log(
        `[mobile-sw] dynamic imports not in this build, skipped: ${result.missingModules.join(', ')}`
      );
    }
  } catch (error) {
